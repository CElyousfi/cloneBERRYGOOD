 ...
 *
 * Loaded twice (UMD, hand-rolled — mirrors public/lib/paieUtils.js):
 *   - In the browser via <script src="lib/quinzaineUtils.js"> → exposes window.QuinzaineUtils
 *   - In node:test via require('./quinzaineUtils.js') → exposes module.exports
 *
 ...
 * top-level name to the global scope. Only window.QuinzaineUtils (browser) and
 * module.exports (node) are exposed. The export object uses the UNIQUE name
 * `__quinzaineUtilsApi` (never `__api`, which collides with caisseUtils).
 */
// @ts-check
'use strict';
...
(function () {
  ...
  const __quinzaineUtilsApi = {
    getEqPrefix,
    computeTransportQuinzaine,
  };

  if (typeof module !== 'undefined' && module.exports) module.exports = __quinzaineUtilsApi;
  if (typeof window !== 'undefined') window.QuinzaineUtils = __quinzaineUtilsApi;
