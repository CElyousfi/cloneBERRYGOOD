 ...
 * Source : `public/lib/lecturePaieExcel.js`. `functions/lib/paie/` en porte une
 * copie OCTET POUR OCTET, imposée par le déploiement : Firebase ne publie que
 * `functions/`, et un `require('../../public/…')` fait crasher TOUTES les Cloud
 * Functions du fichier au chargement, sans qu'aucun test local ne le voie
 * (mémoire projet `backend-jamais-require-public`). Un test de parité compare
 * les deux fichiers ; s'il tombe, on recopie la source.
 *
 * Chargé en <script> classique dans le navigateur : tout est wrappé dans une
 * IIFE, aucun identifiant top-level ne fuite — une collision de nom dans le
 * scope global crashe le boot React (#200, mémoire projet
 * `umd-global-collision-smoke-load`). Un seul global : `window.LecturePaieExcel`.
 *
 * ── AUCUNE I/O ─────────────────────────────────────────────────────────────
...
(function () {
   ...
   *
   * Mêmes règles que `cleRegistre` (coutOuvrierCampagne) et `numKey` (app.jsx) :
  ...
  var __lecturePaieExcelApi = {
    normaliserLibelle: normaliserLibelle,
    cleMatricule: cleMatricule,
    estNombre: estNombre,
    estLigneTotal: estLigneTotal,
    trouverEnTete: trouverEnTete,
    colonne: colonne,
  ...
  if (typeof module !== 'undefined' && module.exports) module.exports = __lecturePaieExcelApi;
  if (typeof window !== 'undefined') window.LecturePaieExcel = __lecturePaieExcelApi;
