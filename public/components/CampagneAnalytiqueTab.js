...
(function () {
  ...
  function PivotView(props) {
    ...
    function tableauPostes(rap) {
      var avec = rap.lignes.filter(function (l) {
        return l.ecartPostes;
      });
      ...
      var cles = avec[0].ecartPostes.map(function (p) {
        return p.cle;
      });
      var libelles = {};
      avec[0].ecartPostes.forEach(function (p) {
        libelles[p.cle] = p.libelle;
      });
      function poste(l, cle) {
        var p = (l.ecartPostes || []).filter(function (x) {
          return x.cle === cle;
        })[0];
        return p || {
          campagne: 0,
      ...
      return React.createElement('div', {
      ...
      }, 'D\'OÙ VIENT L\'ÉCART — par poste, quinzaine par quinzaine'), React.createElement('div', {
        style: {
          padding: '0 16px 6px',
          fontSize: '10px',
          color: C.textSec
        }
      }, 'Les deux ventilations ne se correspondent pas terme à terme : la ' + 'campagne décompose en salaire / prime de fonction / ancienneté, ' + 'l\'écran Quinzaine en MO Récolte / Hors Récolte / Postes Fixes. ' + 'Aligner ces lignes-là serait inventer une correspondance. Seules ' + 'les primes de terrain et les heures sup ont la même définition ' + 'des deux côtés ; tout le reste se déduit par différence des ' + 'totaux — ce qui garantit que la ventilation boucle exactement ' + 'sur l\'écart affiché.'), React.createElement('table', {
        style: {
          width: '100%',
          borderCollapse: 'collapse',
          fontSize: '11px'
        ...
        return React.createElement('tr', {
          ...
          style: Object.assign({}, tdL, estReste ? {
            fontWeight: 700,
            color: C.text
          } : {})
        }, libelles[cle]), avec.map(function (l) {
          var p = poste(l, cle);
          ...
          }, dh(p.campagne)), React.createElement('td', {
            key: l.periode + 'q',
            style: Object.assign({}, td, {
              color: C.textSec
            })
          }, dh(p.quinzaine)), React.createElement('td', {
            key: l.periode + 'e',
    ...
    function panneauRapprochement() {
      ...
      rap.sansSnapshot && rap.sansSnapshot.length ? React.createElement('span', {
        ...
        style: th,
        title: 'Journées-homme du pointage (les demi-journées comptent 0,5).'
      }, 'JH'), React.createElement('th', {
        style: th,
        title: 'JH de l\'écran Quinzaine. Un écart ici est un écart de PÉRIMÈTRE — à régler avant de regarder les dirhams.'
      }, 'JH Quinz.'), React.createElement('th', {
        style: th,
        ...
        style: th,
        title: 'Total du calcul CAMPAGNE, avant passage par la parcelle. Son écart avec la « Grille chargée » mesure ce que la grille ne rattache pas.'
      }, 'Campagne (calc.)'))), React.createElement('tbody', null, rap.lignes.map(function (l, i) {
        return React.createElement('tr', {
          key: l.periode,
          style: {
            background: i % 2 ? C.surface2 : C.surface,
        ...
        }, (Math.round(l.jh * 10) / 10).toLocaleString('fr-MA')), React.createElement('td', {
          style: Object.assign({}, td, {
            ...
            color: l.joursQuinzaine !== null && Math.abs(l.joursQuinzaine - l.jh) > 0.5 ? '#c0392b' : C.textSec,
            fontWeight: l.joursQuinzaine !== null && Math.abs(l.joursQuinzaine - l.jh) > 0.5 ? 700 : 400
          })
        }, l.joursQuinzaine === null ? '—' : (Math.round(l.joursQuinzaine * 10) / 10).toLocaleString('fr-MA')), React.createElement('td', {
          style: td
  ...
  var CONSO_BUCKETS = {
    engrais: {
      items: 'engrais',
      cout: 'totalEngraisCout'
    },
    pesticides: {
      items: 'pesticides',
      cout: 'totalPesticidesCout'
    }
  };
  ...
  function ConsoView(props) {
    ...
    var classing = _classing[0];
    var setClassing = _classing[1];

    // Compte rendu du dernier classement : { article, fiches, erreur }.
    var _classMsg = useState(null);
    var classMsg = _classMsg[0];
    var setClassMsg = _classMsg[1];
    var bucket = CONSO_BUCKETS[subTab] || {
      items: '',
      cout: ''
    };
    ...
    var classer = function (article, categorie) {
      ...
      CAT_classerArticle(article, categorie).then(function (json) {
        setClassMsg({
          article: article,
          fiches: json.fiches_mises_a_jour || 0,
          erreur: ''
        });
        setClassing('');
        ...
        setClassMsg({
          article: article,
          fiches: 0,
          erreur: e.message || 'Échec du classement'
        });
        setClassing('');
      });
    };
    ...
    }, aClasser.length + (aClasser.length > 1 ? ' articles ne sont ni engrais ni pesticide au catalogue' : ' article n\'est ni engrais ni pesticide au catalogue')), React.createElement('div', {
      style: {
        color: C.textSec,
        marginBottom: '8px'
      }
    }, peutClasser ? 'Leurs quantités ne sont comptées dans AUCUN des deux onglets. ' + 'Classez-les ici : la correction porte sur la fiche catalogue, donc sur ' + 'tout l\'historique, sans ressaisir les bons.' : 'Leurs quantités ne sont comptées dans AUCUN des deux onglets. ' + 'Corrigez la catégorie de ces articles dans le catalogue (Stock › Articles) : ' + 'la correction vaut pour tout l\'historique, sans ressaisir les bons.'),
    ...
    classMsg ? React.createElement('div', {
    ...
    }, classMsg.erreur ? classMsg.article + ' : ' + classMsg.erreur : classMsg.article + ' classé — ' + classMsg.fiches + (classMsg.fiches > 1 ? ' fiches mises à jour' : ' fiche mise à jour')) : null, React.createElement('ul', {
      style: {
        margin: 0,
        paddingLeft: '18px'
      }
      ...
      }, React.createElement('strong', null, a.article), ' — ' + a.lignes + (a.lignes > 1 ? ' lignes' : ' ligne') + ', ' + fmtQty(a.quantite) + (a.unite ? ' ' + a.unite : ''), React.createElement('span', {
        style: {
          color: C.textSec
        }
      }, ' (catégorie actuelle : ' + a.categorie_actuelle + ')'), !peutClasser ? null : sansFiche ? React.createElement('span', {
        style: {
          color: C.textSec,
          marginLeft: '6px',
          fontStyle: 'italic'
        }
      }, '— créez d\'abord sa fiche dans Stock › Articles') : React.createElement('span', {
        style: {
          whiteSpace: 'nowrap'
        }
      }, React.createElement('button', {
        onClick: function () {
          classer(a.article, 'engrais');
        },
        disabled: !!classing,
        title: 'Classer « ' + a.article + ' » en engrais',
        style: classBtnStyle(!!classing)
      }, enCours ? '…' : 'Engrais'), React.createElement('button', {
        onClick: function () {
          classer(a.article, 'pesticide');
        },
        disabled: !!classing,
  ...
  function CampagneAnalytiqueTab(props) {
    ...
    useEffect(function () {
      ...
      fetch('/api/pointage-rh?action=cout-quinzaine').then(function (r) {
        return r.json();
      }).then(function (d) {
        if (cancelled || !d || !d.success) return;
    ...
    var loadConso = function () {
      ...
      setConsoErr(null);
      return fetch('/api/pointage-rh?action=campagne-conso-parcelle').then(function (r) {
        return r.json();
      }).then(function (d) {
        if (d && d.success) {
          setConsoData(d);
