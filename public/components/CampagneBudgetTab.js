...
(function () {
  ...
  function CBT_porteeOptions(args) {
    ...
    (a.rows || []).forEach(function (r) {
      ...
      if (!parVariete[key]) {
        parVariete[key] = {
          culture: culture,
          variete: CBT_normVariete(r && r.variete),
          nb: 0
        };
      }
      parVariete[key].nb += 1;
  ...
  function CBT_targetLabels(args) {
    ...
    return Object.keys(vus).sort().map(function (up) {
      return vus[up];
    });
  }
  ...
  function CBT_surfaceCible(labels, haOf) {
    ...
    var fn = typeof haOf === 'function' ? haOf : function () {
      return 0;
    };
    var ha = 0;
    var sansHa = [];
  ...
  function CBT_fanoutEcrasements(args) {
    ...
    (a.labels || []).forEach(function (label) {
      ...
      function ajoute(champ, avant, apres) {
        ...
        nb += 1;
        if (exemples.length < 3) exemples.push({
          champ: champ,
          avant: avant,
          apres: apres
        });
  ...
  function CBT_fanoutMessage(res, nbDemandes, nomOf) {
    ...
    var nom = typeof nomOf === 'function' ? nomOf : function (l) {
      return String(l);
    };
    ...
    var kos = results.filter(function (r) {
      return !(r && r.ok);
    });
    if (kos.length > 0) {
      return {
        type: 'ko',
        text: oks.length + '/' + n + ' parcelles enregistrées — ' + kos.length + ' en échec : ' + liste(kos.map(function (r) {
          return String(r && r.label_bee_one || '?');
        })) + '. Rafraîchir avant de réessayer.'
      };
    }
    var base = 'Budget enregistré sur ' + oks.length + (oks.length > 1 ? ' parcelles' : ' parcelle');
    ...
    oks.forEach(function (r) {
      ...
      var purges = (Array.isArray(r.familles_purgees) ? r.familles_purgees.length : 0) + (Array.isArray(r.operations_purgees) ? r.operations_purgees.length : 0) + (Array.isArray(r.quinzaines_purgees) ? r.quinzaines_purgees.length : 0) + (Array.isArray(r.quinzaines_supprimees) ? r.quinzaines_supprimees.length : 0) + CBT_num(r.purge_differee);
    ...
    if (avecNeutralisation.length > 0) {
      parts.push('valeur de famille remplacée par le détail des opérations sur ' + avecNeutralisation.length + ' parcelle' + (avecNeutralisation.length > 1 ? 's' : '') + ' : ' + liste(avecNeutralisation));
    }
    if (avecPurge.length > 0) {
      parts.push('entrées obsolètes retirées sur ' + avecPurge.length + ' parcelle' + (avecPurge.length > 1 ? 's' : '') + ' : ' + liste(avecPurge));
    }
    return {
      type: 'ok',
      purge: true,
  ...
  function CampagneBudgetTab(props) {
    ...
    var portee = _portee[0];
    var setPortee = _portee[1];
    var _varieteSel = useState('');
    var varieteSel = _varieteSel[0];
    var setVarieteSel = _varieteSel[1];
    var _cultureSel = useState('');
    var cultureSel = _cultureSel[0];
    var setCultureSel = _cultureSel[1];
    ...
    var confirmFanout = _confirmFanout[0];
    var setConfirmFanout = _confirmFanout[1];
    ...
    var rowsAffichables = useMemo(function () {
      ...
      return (rows || []).filter(function (r) {
        return CBT_parcelleAffichable(r, sbMap);
      });
    }, [rows]);
    ...
    var porteeOpts = useMemo(function () {
      return CBT_porteeOptions({
        rows: rowsAffichables,
    ...
    useEffect(function () {
      if (varieteSel && !porteeOpts.varietes.some(function (v) {
        return v.key === varieteSel;
      })) {
        setVarieteSel('');
      }
      if (cultureSel && !porteeOpts.cultures.some(function (c) {
        return c.key === cultureSel;
      })) {
        setCultureSel('');
      }
    ...
    useEffect(function () {
      ...
      setValues(next);
      setOpValues(nextOps);
    }, [porteeMulti, communCibles, selected, familles, opsByFamille, budgetsByLabel, opBudgetsByLabel]);
    ...
    porteeOpts.varietes.forEach(function (v) {
      if (v.key === varieteSel) varieteBucket = v;
    });
    ...
    var culture = porteeMulti ? portee === 'culture' ? cultureSel : varieteBucket ? varieteBucket.culture : '' : selectedRow ? cbtCulture(selectedRow.culture, selectedRow.label) : '';
    ...
    var quinzaineSaisissable = !!(quinzaineActive && CBT_quinzaineApplicable(culture)) && !porteeMulti;
    ...
    function famillesEnJeu() {
      ...
      return familles.filter(function (f) {
        return !!touched[f];
      });
    }
    ...
    function handleSaveFanout(confirme) {
      ...
      fetch('/api/pointage-rh?action=campagne-budget-save', {
        ...
        if (oks.length > 0) {
          ...
          setQuinzByLabel(function (prev) {
            ...
            oks.forEach(function (r) {
              ...
              next[key] = CBQ ? CBQ.quinzainesByLabel([{
                label_bee_one: key,
                budgets_quinzaine: r.budgets_quinzaine || {}
              }])[key] || {} : r.budgets_quinzaine || {};
            });
            return next;
          });
        }
        ...
        setFanoutResults(results.some(function (r) {
          return !(r && r.ok);
        }) ? results : null);
        ...
        setMsg({
          type: 'ko',
          text: 'Enregistrement interrompu (' + e.message + ') — des parcelles ont' + ' peut-être été enregistrées, état inconnu. L\'écran est rechargé.'
        });
        setTick(function (t) {
          return t + 1;
        });
    ...
    function libelleDivergence(d, toucheeFamille) {
      return d.nb + ' valeurs différentes (' + d.min + ' → ' + d.max + ') — ' + (toucheeFamille ? 'à saisir avant d\'enregistrer' : 'non modifiée à l\'enregistrement');
    }
    var thStyle = {
      textAlign: 'left',
      padding: '8px 10px',
    ...
    portee === 'variete' && React.createElement('select', {
      ...
      value: ''
    }, '— Choisir une variété —'), porteeOpts.varietes.map(function (v) {
      return React.createElement('option', {
        key: v.key,
        value: v.key
      ...
      value: ''
    }, '— Choisir une culture —'), porteeOpts.cultures.map(function (c) {
      return React.createElement('option', {
        key: c.key,
        value: c.key
      }, c.label + ' (' + c.nb + (c.nb > 1 ? ' parcelles)' : ' parcelle)'));
    })), culture && React.createElement(CBT_CultureBadge, {
      culture: culture
    }), !porteeMulti && selectedRow && React.createElement('span', {
      style: {
        fontSize: 12,
        color: CBT_C.textSec
      }
    ...
    }, React.createElement('div', null, React.createElement('strong', null, 'Portée : ' + (porteeCibleLabel || '—')), targetLabels.length === 0 ? ' — aucune parcelle : choisir une cible ci-dessus.' : ' — ' + targetLabels.length + (targetLabels.length > 1 ? ' parcelles' : ' parcelle') + ' — ' + surfaceCible.ha.toFixed(2) + ' ha' + (surfaceCible.sansHa.length > 0 ? ' (' + surfaceCible.sansHa.length + ' sans Ha : ' + porteeSansHaNoms + ' — leur Total JH n\'est pas compté, leur budget l\'est)' : '')), React.createElement('div', {
      style: {
        color: CBT_C.textTer
      }
    }, 'Engagement quinzaine : saisie par parcelle uniquement.')), !grilleVisible && React.createElement('div', {
      style: {
        padding: '28px 0',
        textAlign: 'center',
        color: CBT_C.textTer,
    ...
    }, porteeMulti ? portee === 'variete' ? 'Choisir une variété pour saisir son budget.' : 'Choisir une culture pour saisir son budget.' : 'Sélectionner une parcelle pour saisir son budget.'), grilleVisible && React.createElement('div', {
      style: {
        background: CBT_C.surface,
        border: '1px solid ' + CBT_C.border,
        borderRadius: 12,
    ...
    familles.map(function (f, i) {
      ...
      var ecrase = calcule && CBT_num(values[f]) > 0 && (!porteeMulti || !!touched[f]);
      ...
      divergentes[f] && React.createElement('span', {
        style: badgeAmbre
      }, React.createElement('i', {
        className: 'fa-solid fa-triangle-exclamation',
        style: {
      ...
      }), libelleDivergence(divergentes[f], !!touched[f]))), React.createElement('td', {
        style: {
          ...tdStyle,
          textAlign: 'right'
        }
      ...
      if (isOpen) {
        ops.forEach(function (op) {
          ...
          }), libelleDivergence(divergentesOps[f][op], !!touched[f]))), React.createElement('td', {
            style: {
              ...tdStyle,
              textAlign: 'right'
            }
    ...
    canEdit && (confirmList && confirmList.length > 0 || confirmQuinz && confirmQuinz.length > 0 || confirmFanout) && React.createElement('div', {
      style: {
        padding: '12px 14px',
    ...
    confirmFanout && React.createElement('div', {
    ...
    }), 'Enregistrer la même grille sur ' + confirmFanout.labels.length + (confirmFanout.labels.length > 1 ? ' parcelles' : ' parcelle') + (confirmFanout.ecrasements.length > 0 ? ' — ' + confirmFanout.ecrasements.length + (confirmFanout.ecrasements.length > 1 ? ' ont des valeurs qui seront remplacées :' : ' a des valeurs qui seront remplacées :') : ' (aucune valeur existante remplacée) :')), confirmFanout && React.createElement('ul', {
      style: {
        margin: '0 0 10px',
        paddingLeft: 26
      }
      ...
      var hit = null;
      confirmFanout.ecrasements.forEach(function (e) {
        if (e.label === lbl) hit = e;
      });
      ...
      }, cbtNom(lbl), hit && React.createElement('span', {
        style: {
          fontWeight: 700
        }
      }, ' — ' + hit.nb + (hit.nb > 1 ? ' valeurs remplacées' : ' valeur remplacée') + ' : ' + hit.exemples.map(function (x) {
        return x.champ + ' ' + x.avant + ' → ' + (x.apres > 0 ? String(x.apres) : '0 (supprimé)');
      }).join(', ') + (hit.nb > hit.exemples.length ? '…' : '')));
    ...
    }, !canEdit ? 'Saisie réservée aux profils DG/RH.' : porteeMulti ? 'Grille commune aux parcelles de la portée : ce qui concorde est' + ' pré-rempli, ce qui diverge reste vide et est signalé.' + ' Seules les lignes que vous modifiez sont propagées ; les autres' + ' restent inchangées sur chaque parcelle.' : 'Déplier une famille pour saisir ses opérations. Le total de famille' + ' devient calculé dès qu\'une opération est budgétée ; sinon il reste' + ' saisissable. Un champ vide (ou 0) supprime la ligne.')),
    ...
    fanoutResults && React.createElement('div', {
    ...
    }, 'Résultat par parcelle'), React.createElement('ul', {
      style: {
        margin: 0,
        paddingLeft: 20
      }
      ...
      return React.createElement('li', {
        ...
        style: {
          marginBottom: 2,
          color: r && r.ok ? CBT_C.green : '#dc2626'
        }
      }, (r && r.ok ? '✓ ' : '✗ ') + cbtNom(lbl) + (r && r.ok ? '' : ' — ' + String(r && r.error || 'échec')));
    }))))));
  }
