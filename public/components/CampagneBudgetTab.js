...
(function () {
  ...
  function CampagneBudgetTab(props) {
    ...
    familles.map(function (f, i) {
      ...
      }), divergentes[f].nb + ' valeurs différentes (' + divergentes[f].min + ' → ' + divergentes[f].max + ') — non modifiée à l\'enregistrement')), React.createElement('td', {
        style: {
          ...tdStyle,
          textAlign: 'right'
        }
      ...
      if (isOpen) {
        ops.forEach(function (op) {
          ...
          }), divergentesOps[f][op].nb + ' valeurs différentes (' + divergentesOps[f][op].min + ' → ' + divergentesOps[f][op].max + ') — non modifiée à l\'enregistrement')), React.createElement('td', {
            style: {
              ...tdStyle,
              textAlign: 'right'
            }
    ...
    }, !canEdit ? 'Saisie réservée aux profils DG/RH.' : porteeMulti ? 'Grille commune aux parcelles de la portée : ce qui concorde est' + ' pré-rempli, ce qui diverge reste vide et est signalé.' + ' L\'enregistrement en portée multiple arrive avec le lot suivant.' : 'Déplier une famille pour saisir ses opérations. Le total de famille' + ' devient calculé dès qu\'une opération est budgétée ; sinon il reste' + ' saisissable. Un champ vide (ou 0) supprime la ligne.')))));
  }
