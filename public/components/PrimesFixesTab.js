...
(function () {
  ...
  function PrimesFixesTab() {
    ...
    addOpen ? c('div', {
      ...
      onClick: function (e) {
        if (e.target === e.currentTarget && !addSaving) closeAdd();
      }
    }, c('div', {
      style: {
        background: '#fff',
    ...
    }))), !addSelected ? c('div', null, c('div', {
      style: {
        fontSize: 12,
        color: '#666',
        marginBottom: 8
      }
    }, 'Rechercher dans toute la base (matricule ou nom) — y compris les ouvriers sans prime.'), c('input', {
      type: 'text',
      value: addSearch,
      autoFocus: true,
    ...
    }, 'Saisissez un matricule ou un nom pour rechercher.') : addResults.length === 0 ? c('div', {
      style: {
        color: '#999',
        fontSize: 13,
        padding: 12,
      ...
      return c('div', {
        ...
        onClick: function () {
          setAddSelected(w);
          setAddMontant(w.prime > 0 ? String(w.prime) : '');
          setAddError('');
        },
        style: {
          display: 'flex',
      ...
      }, c('span', null, c('strong', null, w.matricule), '  ', w.nom || c('span', {
        style: {
          color: '#bbb'
        }
      }, '(sans nom)')), c('span', {
        style: {
          color: w.prime > 0 ? 'var(--berry)' : '#bbb',
          fontWeight: 600
        }
      }, w.prime > 0 ? PFT_fmt(w.prime) + ' DH' : '—'));
    }))) : c('div', null, c('div', {
      style: {
        background: '#f8f5fb',
        borderRadius: 8,
    ...
    }, addSelected.matricule, ' — ', addSelected.nom || '(sans nom)'), c('div', {
      style: {
        fontSize: 12,
        color: '#666',
        marginTop: 2
      }
    }, 'Prime actuelle : ', addSelected.prime > 0 ? PFT_fmt(addSelected.prime) + ' DH/jour' : 'aucune (0)'), c('button', {
      onClick: function () {
        setAddSelected(null);
      },
      disabled: addSaving,
    ...
    }, 'Prime de fonction (DH/jour)'), c('input', {
      type: 'text',
      inputMode: 'decimal',
      value: addMontant,
      autoFocus: true,
    ...
    }, '= ', PFT_fmt(PFT_parse(addMontant)), ' DH/jour'), c('label', {
      style: {
        display: 'block',
        fontSize: 12,
        color: '#444',
    ...
    }, addSaving ? 'Enregistrement…' : 'Enregistrer la prime'))))) : null,
    ...
    histRow ? c('div', {
      ...
      onClick: function (e) {
        if (e.target === e.currentTarget) setHistRow(null);
      }
    }, c('div', {
      style: {
        background: '#fff',
    ...
    }), 'Historique — ', histRow.matricule, ' ', histRow.nom || ''), c('button', {
      onClick: function () {
        setHistRow(null);
      },
      style: {
    ...
    }, 'Aucun historique avant la 1re modification dans l\'app') : c('table', {
      style: {
        width: '100%',
        borderCollapse: 'collapse',
        fontSize: 13
    ...
    }, 'Auteur'))), c('tbody', null, histView.map(function (e, i) {
      return c('tr', {
        key: i,
        style: {
          borderBottom: '1px solid #f2f2f2'
      ...
      }, PFT_fmtDateTime(e.changedAt)), c('td', {
        style: {
          padding: '6px 8px',
          textAlign: 'right'
        }
      ...
      }, PFT_fmt(e.montant))), c('td', {
        style: {
          padding: '6px 8px'
        }
      }, e.effectiveFrom || '—'), c('td', {
        style: {
          padding: '6px 8px'
        }
      }, e.author));
