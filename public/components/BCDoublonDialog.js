 ...
 *
 * Chargé en <script> séparé (build Babel). PARTAGE LE SCOPE GLOBAL : tout est
 * wrappé dans une IIFE et n'expose QU'UN global au nom unique
 * (window.BCDoublonDialog) — anti-collision UMD (cf. crashes #200). Les
 * identifiants internes sont préfixés BCD_.
 *
...
(function () {
  ...
  function BCDoublonDialog(props) {
    ...
    numero ? React.createElement('div', {
      style: {
        padding: '10px 12px',
        borderRadius: 8,
        background: '#fdecea',
    ...
    }, 'Bon existant : ', React.createElement('strong', {
      style: {
        fontSize: 15
      }
    }, numero)) : null, React.createElement('div', {
      style: {
        fontSize: 12,
        color: 'var(--gray-400)',
        marginBottom: 8
    ...
    doublon.message ? React.createElement('div', {
      style: {
        fontSize: 12,
        color: '#555',
        marginBottom: 14
      }
    }, doublon.message) : null, React.createElement('div', {
      style: {
        fontSize: 11,
        color: 'var(--gray-400)',
        marginBottom: 14
      }
    }, 'Vérifiez le bon ', numero || 'existant', ' dans la liste avant de continuer. Si vous créez quand même ce bon, votre nom et la raison du blocage seront enregistrés.'), React.createElement('div', {
      style: {
        display: 'flex',
        gap: 8,
  ...
  window.BCDoublonDialog = BCDoublonDialog;
})();
