...
(function () {
  ...
  function groupOuvriersByParcelle(ouvriers) {
    ...
    groups.forEach(function (g) {
      g.ouvriers.sort(function (a, b) {
        return (a.nom || '').localeCompare(b.nom || '');
      });
    });
    groups.sort(function (a, b) {
      return b.ouvriers.length - a.ouvriers.length;
    });
    return groups;
  }
  ...
  function PointageValidationPanel(props) {
    ...
    var refetch = useCallback(function () {
      ...
      setLoading(true);
      setErr(null);
      window.fetch('/api/pointage-validation?action=get-validations&date=' + encodeURIComponent(date)).then(function (r) {
        return r.json();
      }).then(function (j) {
        if (j && j.success) setValidation(j.validation || {
          date: date,
          fermes: {}
    ...
    function post(action, body, key) {
      ...
      return window.fetch('/api/pointage-validation?action=' + action, {
        ...
        if (res.j && res.j.success) {
          ...
          setErr(res.j && res.j.error || 'Erreur');
        }
        return res;
      }).catch(function (e) {
        setErr(e.message);
    ...
    }, 'Validation du pointage'), loading && React.createElement('i', {
    ...
    }, err), fermes.map(function (ferme) {
      return renderFerme(ferme);
    }));
    function renderFerme(ferme) {
      var fdata = equipesParFerme[ferme] || {
        equipes: [],
        diversCount: 0
      };
      ...
      if (submitState === 'valide' || locked) {
        stateLabel = 'Validé — figé';
        stateColor = '#137333';
        stateBg = '#e6f4ea';
      } else if (submitState === 'soumis') {
        stateLabel = 'En attente validation Chef';
        stateColor = '#b06000';
        stateBg = '#fef3e0';
      } else {
      ...
      // submit autorisé ?
      var equipeIds = equipesList.map(function (e) {
        return e.prefix;
      });
      ...
      }, stateLabel), React.createElement('span', {
        style: {
          flex: 1
        }
      }), React.createElement('span', {
      ...
      }, equipesList.length + ' équipe' + (equipesList.length > 1 ? 's' : ''))), React.createElement('div', {
        style: {
          padding: '10px 14px'
        }
      }, equipesList.length === 0 ? React.createElement('div', {
        style: {
          fontSize: 12,
          color: '#9aa0a6',
          fontStyle: 'italic'
        }
      }, 'Aucune équipe ce jour.') : equipesList.map(function (eq) {
        return renderEquipe(ferme, eq, fstate, canRHEdit);
      }), renderDivers(ferme, fdata, fstate, canRHEdit), renderActions(ferme, fstate, submitState, locked, canSubmit, equipeIds)));
    }
    function renderEquipe(ferme, eq, fstate, canRHEdit) {
      ...
      }, eq.nom || prefix), React.createElement('span', {
        style: {
      ...
      }, ouvriers.length + ' ouv.'), eq.coutTransport != null && React.createElement('span', {
        style: {
          fontSize: 11,
          color: '#9aa0a6'
        }
      }, 'Transport ' + fmtMoney(eq.coutTransport) + ' DH'), React.createElement('span', {
        style: {
          background: badge.bg,
          color: badge.color,
          padding: '2px 8px',
      ...
      isOpen && React.createElement('div', {
      ...
      }, 'Matricule'), React.createElement('th', {
        style: {
          padding: '6px 10px'
        }
      ...
      }, 'Tâche'), React.createElement('th', {
        style: {
          padding: '6px 10px',
          textAlign: 'center'
        }
      }, 'Heures'))), React.createElement('tbody', null, ouvriers.length === 0 ? React.createElement('tr', null, React.createElement('td', {
        colSpan: 4,
        style: {
          padding: '8px 10px',
          color: '#9aa0a6',
      ...
      }, 'Aucun ouvrier.')) : groupOuvriersByParcelle(ouvriers).map(function (g, gi) {
        var rows = [];
        ...
        }), g.parcelle, React.createElement('span', {
          style: {
            fontWeight: 600,
            color: '#9aa0a6',
            marginLeft: 8
        ...
        g.ouvriers.forEach(function (r, i) {
          ...
          }, r.matricule), React.createElement('td', {
            style: {
              fontWeight: 600,
              padding: '6px 10px'
            }
          }, r.nom), React.createElement('td', {
            style: {
              fontSize: 11,
              color: '#5f6368',
              padding: '6px 10px'
      ...
      canRHEdit && isOpen && React.createElement('div', {
        ...
        value: motifVal,
        onChange: function (e) {
          setMotif(ferme, prefix, e.target.value);
        },
        style: {
          flex: 1,
          minWidth: 120,
    ...
    function renderDivers(ferme, fdata, fstate, canRHEdit) {
      ...
      }, 'Pointage Divers'), count > 0 && React.createElement('span', {
        style: {
          fontSize: 11,
          color: '#9aa0a6'
        }
      }, count + ' entrée' + (count > 1 ? 's' : '')), React.createElement('span', {
        style: {
          background: badge.bg,
          color: badge.color,
          padding: '2px 8px',
    ...
    function renderActions(ferme, fstate, submitState, locked, canSubmit, equipeIds) {
        ...
        rows.push(React.createElement('button', {
          ...
          onClick: function () {
            post('submit-ferme', {
              date: date,
              ferme: ferme,
              equipesDuJour: equipeIds
            }, ferme + '|submit');
          },
          style: {
            padding: '10px 16px',
            background: canSubmit ? 'var(--berry)' : 'var(--gray-200)',
      ...
      if (chefFerme === ferme && submitState === 'soumis') {
        ...
        rows.push(React.createElement('input', {
          key: 'rejmotif',
          type: 'text',
          placeholder: 'Motif du renvoi (optionnel)',
          value: rejMotif,
          onChange: function (e) {
            setMotif(ferme, '__chefreject__', e.target.value);
          },
          style: {
            padding: '8px 12px',
            border: '1px solid var(--gray-200)',
      ...
      if (isDG && (submitState === 'valide' || locked)) {
        rows.push(React.createElement('button', {
          ...
          disabled: !!busy,
          onClick: function () {
            if (window.confirm('Déverrouiller le pointage de ' + ferme + ' ? La saisie RH sera rouverte.')) {
              post('unlock-ferme', {
                date: date,
                ferme: ferme
              }, ferme + '|unlock');
