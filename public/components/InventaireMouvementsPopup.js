...
(function () {
  ...
  function IMP_sanitizeName(s) {
    return String(s || '').replace(/[/\\:*?[\]]/g, '_').replace(/\s+/g, '_').replace(/_+/g, '_').replace(/^_+|_+$/g, '') || 'NA';
  }
  ...
  function IMP_exportExcel(opts) {
    ...
    var sub = articleNom + (lieuId ? ' · ' + lieuId : '') + (unite ? ' · ' + unite : '') + (dateInventaire ? ' · solde au ' + IMP_fmtDateFr(dateInventaire) : '');
    var aoa = [];
    ...
    rows.forEach(function (r) {
      ...
      aoa.push([IMP_fmtDateFr(r.date), typeLbl, r.lieu_id || '—', r.quantite,
      // Number (signé +/−)
      r.solde_courant // Number
      ]);
    });
    aoa.push([]);
    aoa.push([rows.length + ' mouvements', '', '', '', 'Solde final : ' + IMP_fmtNum(soldeFinal, 2) + (unite ? ' ' + unite : '')]);
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      var qAddr = XLSX.utils.encode_cell({
        r: rIdx,
        c: 3
      });
      var sAddr = XLSX.utils.encode_cell({
        r: rIdx,
        c: 4
      });
      if (ws[qAddr]) {
  ...
  function InventaireMouvementsPopup(props) {
    ...
    }, headerSub)), React.createElement('div', {
      style: {
        display: 'flex',
        alignItems: 'center',
        gap: 10
    ...
    }, 'Exporter Excel'), React.createElement('button', {
      onClick: onClose,
      style: {
        border: 'none',
