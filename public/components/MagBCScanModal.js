...
(function () {
  ...
  function MagBCScanModal({
    ...
    const [lotLieu, setLotLieu] = useState({
      type: 'magasin',
      id: magasins[0]
    });
    ...
    const selectableParcelleRefs = () => useConsoSelector ? refParcelles.map(p => ({
      label: p.label,
      nom: nomOf(p.label),
      culture: p.culture || ''
    })) : allParcelles.map(p => ({
      label: p.Parcelle_Physique,
      nom: nomOf(p.Parcelle_Physique),
      culture: p.Culture || ''
    }));
    ...
    const resolveParcelle = val => {
      ...
      if (ref) return {
        ...
        culture: cultureOf(val, ref.culture),
        ferme: ref.ferme || '',
        groupe_id: ''
      };
      ...
      return {
        parcelle: val,
        parcelle_ref: '',
        culture: cultureOf(val, parc && parc.Culture),
        ferme: parc && parc.Ferme || '',
        groupe_id: ''
      };
    };
    ...
    const emptyLine = () => ({
      ...
      article: '',
      article_initial: '',
      article_status: 'unmatched',
      article_alias_count: undefined,
      parcelle_lue: '',
      parcelle_status: 'unmatched',
      parcelle_candidats: [],
      parcelle_initial: '',
      parcelle_alias_count: undefined,
      parcelle_score: undefined,
      quantite: '',
      unite: 'kg',
      parcelle: '',
      parcelle_ref: '',
    ...
    const handleFiles = files => {
      ...
      if (!valid.length) {
        alert('Veuillez déposer des images (JPG, PNG)');
        return;
      }
      const base = Date.now();
      const entries = valid.map((f, i) => ({
        ...
        header: {
          ...
          motif: '',
          remarques: '',
          lieu_source_type: lotLieuRef.current.type,
          lieu_source_id: lotLieuRef.current.id
        },
        items: []
      }));
    ...
    useEffect(() => {
      ...
      const worker = async () => {
        for (;;) {
          ...
          if (!next) break;
          const id = next.id;
          const markProcessing = q => q.id === id ? {
            ...q,
            status: 'processing'
          } : q;
    ...
    useEffect(() => {
      ...
      if (!camp) {
        parcelleAliasesRef.current = {};
        return;
      }
      let annule = false;
      fetch('/api/stock?action=list-bc-scan-parcelle-aliases&campagne=' + encodeURIComponent(camp)).then(r => r.json()).then(j => {
        if (!annule && j && j.success && j.aliases) parcelleAliasesRef.current = j.aliases;
      }).catch(() => {});
      return () => {
        annule = true;
      };
    ...
    const addLine = () => {
      ...
      setQueue(prev => prev.map(q => q.id === id ? {
        ...q,
        items: q.items.concat([emptyLine()])
      } : q));
    };
    ...
    const changeLotLieu = nextLieu => {
      setLotLieu(nextLieu);
      setQueue(prev => prev.map(q => q.status === 'saved' ? q : {
        ...q,
        header: {
          ...q.header,
          lieu_source_type: nextLieu.type,
          lieu_source_id: nextLieu.id
        }
      }));
    };
    ...
    const isActiveLine = it => !ligneExclue(it) && !!((it.article_lu || '').trim() || (it.article || '').trim() || String(it.quantite || '').trim() || (it.parcelle || '').trim());
    ...
    const saveAlias = (libelleLu, articleNom) => fetch('/api/stock?action=save-bc-scan-alias', {
      ...
      body: JSON.stringify({
        ...
        created_by: {
          ...
          name: profileData && profileData.name || currentProfile
        }
      })
    }).catch(() => {});
      ...
      try {
        ...
        const resp = await fetch('/api/stock?action=create-bc', {
          ...
          body: JSON.stringify({
            ...
            lieu_source: {
              type: entry.header.lieu_source_type,
              id: entry.header.lieu_source_id
            },
        ...
        if (campagne) {
          validItems.filter(i => (i.parcelle_lue || '').trim() && !i.groupe_id && knownParcelle(i.parcelle) && i.parcelle !== i.parcelle_initial).forEach(i => {
            saveParcelleAlias(i.parcelle_lue, i.parcelle);
          });
        }
        ...
        try {
          saveJournal(entry.header.date, json.numero || '', validItems);
        } catch (e) {}
        const savedId = entry.id;
        setQueue(prev => prev.map(q => q.id === savedId ? {
          ...q,
          status: 'saved',
          numero: json.numero || ''
        } : q));
    ...
    const handleClose = () => {
      if (processing) {
        if (!confirm('Une analyse est en cours. Fermer quand même ?')) return;
      }
    ...
    const statusDot = (status, aliasCount, quoi) => {
      if (status === 'exact') return /*#__PURE__*/React.createElement("span", {
        title: "Reconnu au r\xE9f\xE9rentiel",
        style: {
          color: '#2e7d32',
          marginLeft: 4
        }
      ...
      if (status === 'alias') {
        ...
        return /*#__PURE__*/React.createElement("span", {
          title: 'Correction mémorisée lors d\'un scan précédent — vérifiez que ' + (quoi || 'l\'article est le bon') + suffixe,
          style: {
            color: '#e65100',
            marginLeft: 4,
            fontSize: 10,
        ...
        }, "\u26A0\uFE0F m\xE9moris\xE9 \u2014 \xE0 v\xE9rifier");
      }
      if (status === 'probable') return /*#__PURE__*/React.createElement("span", {
        title: "\xC0 v\xE9rifier",
        style: {
          color: '#e65100',
          marginLeft: 4
        }
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: {
        background: 'linear-gradient(135deg, #bf360c 0%, ' + BCSCAN_ORANGE + ' 100%)',
        padding: '20px 24px',
        color: '#fff',
    ...
    }), "Scanner des Bons de Consommation"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 12,
        opacity: 0.85,
        marginTop: 4
      }
    }, "D\xE9posez les photos des bons papier \u2014 l'IA extrait les lignes, vous corrigez et enregistrez bon par bon")), /*#__PURE__*/React.createElement("button", {
      onClick: handleClose,
      title: "Fermer",
      style: {
      ...
      style: labelStyle
    }, "Lieu de d\xE9part du lot *"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        gap: 6
      }
      ...
      value: lotLieu.type,
      onChange: e => changeLotLieu({
        type: e.target.value,
        id: e.target.value === 'magasin' ? magasins[0] : stations[0]
      }),
      style: {
        ...fStyle,
        width: 'auto'
    ...
    }, (lotLieu.type === 'magasin' ? magasins : stations).map(l => /*#__PURE__*/React.createElement("option", {
      key: l,
      value: l
    ...
    }, "Appliqu\xE9 \xE0 tous les bons du lot \u2014 modifiable ensuite bon par bon."), queue.length > 0 && /*#__PURE__*/React.createElement("button", {
      onClick: () => {
      ...
      style: {
        padding: '7px 14px',
        borderRadius: 8,
        border: '1.5px solid ' + BCSCAN_ORANGE,
        background: '#fff',
        color: BCSCAN_ORANGE,
        cursor: 'pointer',
        fontWeight: 600,
    ...
    }), "Ajouter des photos")), queue.length === 0 && /*#__PURE__*/React.createElement("div", {
      onClick: () => {
        const inp = document.createElement('input');
        inp.type = 'file';
        inp.multiple = true;
      ...
      onDragOver: e => {
        e.preventDefault();
        e.currentTarget.style.borderColor = BCSCAN_ORANGE;
        e.currentTarget.style.background = 'rgba(230,81,0,0.06)';
      },
      onDragLeave: e => {
        e.currentTarget.style.borderColor = '#ffccbc';
    ...
    }, "Glissez vos photos de bons ici"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 13,
        color: '#888'
      }
    ...
    }, "cliquez pour s\xE9lectionner"), " \u2014 plusieurs images \xE0 la fois"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11,
        color: '#aaa',
        marginTop: 8
      }
    }, "JPG, PNG \u2014 les images sont r\xE9duites avant envoi")), queue.length > 0 && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        gap: 8,
        alignItems: 'center',
    ...
    }, "\u2039 Pr\xE9c\xE9dent"), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 13,
        fontWeight: 700
      }
    }, "Bon ", currentIdx + 1, " / ", queue.length), /*#__PURE__*/React.createElement("button", {
      ...
      style: {
        ...
        border: '1px solid #ddd',
        background: '#fff',
        cursor: currentIdx >= queue.length - 1 ? 'not-allowed' : 'pointer',
        fontSize: 12,
        opacity: currentIdx >= queue.length - 1 ? 0.5 : 1
      }
    }, "Suivant \u203A"), processing && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 12,
        color: BCSCAN_ORANGE
      }
    ...
    }, createdCount, " bon(s) enregistr\xE9(s)")), /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        gap: 8,
        overflowX: 'auto',
      ...
      style: {
        ...
        flex: '0 0 auto',
        cursor: 'pointer',
        border: i === currentIdx ? '3px solid ' + BCSCAN_ORANGE : '2px solid #eee',
        borderRadius: 8,
        padding: 2,
        background: '#fff'
      }
    ...
    }, q.status === 'pending' && /*#__PURE__*/React.createElement("span", {
      title: "En attente",
      style: {
        color: '#888'
      }
    }, "\u23F3"), q.status === 'processing' && /*#__PURE__*/React.createElement("i", {
      className: "fa-solid fa-spinner fa-spin",
      style: {
        color: BCSCAN_ORANGE
      }
    }), q.status === 'done' && /*#__PURE__*/React.createElement("span", {
      title: "Analys\xE9",
      style: {
        color: '#2e7d32'
      }
    }, "\u25CF"), q.status === 'saved' && /*#__PURE__*/React.createElement("span", {
      title: "Enregistr\xE9",
      style: {
        color: '#2e7d32'
      }
    }, "\u2705"), q.status === 'error' && /*#__PURE__*/React.createElement("span", {
      title: q.error,
      style: {
        color: '#dc2626'
      }
    }, "\u274C"))))), allSaved && /*#__PURE__*/React.createElement("div", {
      style: {
        padding: '14px 16px',
        background: '#e8f5e9',
    ...
    }), "Lot termin\xE9 \u2014 ", createdCount, " bon(s) de consommation cr\xE9\xE9(s) :", ' ' + queue.map(q => q.numero).filter(Boolean).join(', ')), current && /*#__PURE__*/React.createElement("div", {
      style: {
        border: '2px solid ' + (current.status === 'saved' ? '#4caf50' : current.status === 'error' ? '#e57373' : '#e0e0e0'),
        borderRadius: 12,
        overflow: 'hidden'
      }
    }, /*#__PURE__*/React.createElement("div", {
      style: {
        background: current.status === 'saved' ? '#e8f5e9' : '#fafafa',
        padding: '8px 14px',
        display: 'flex',
        justifyContent: 'space-between',
        alignItems: 'center',
    ...
    }, current.file.name), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 11,
        fontWeight: 600,
        color: current.status === 'saved' ? '#2e7d32' : current.status === 'error' ? '#dc2626' : '#888'
      }
    }, current.status === 'pending' && 'En attente d\'analyse', current.status === 'processing' && 'Analyse…', current.status === 'done' && 'Analysé — à vérifier', current.status === 'saved' && 'Enregistré' + (current.numero ? ' — ' + current.numero : ''), current.status === 'error' && 'Erreur : ' + (current.error || ''))), /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        gap: 0,
        flexWrap: 'wrap'
    ...
    }), "Agrandir")), /*#__PURE__*/React.createElement("div", {
      style: {
        flex: 1,
        minWidth: 320,
        padding: 14
      ...
      type: "date",
      value: current.header.date || '',
      disabled: readOnly,
      onChange: e => patchHeader('date', e.target.value),
      style: fStyle
    })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
      style: labelStyle
    }, "N\xB0 du bon papier"), /*#__PURE__*/React.createElement("input", {
      value: current.header.ref_bon_physique || '',
      disabled: readOnly,
      onChange: e => patchHeader('ref_bon_physique', e.target.value),
      placeholder: "N\xB0 lu sur le bon",
      style: fStyle
    })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
      style: labelStyle
    }, "Motif"), /*#__PURE__*/React.createElement("input", {
      value: current.header.motif || '',
      disabled: readOnly,
      onChange: e => patchHeader('motif', e.target.value),
      style: fStyle
    })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
      style: labelStyle
      ...
      value: current.header.lieu_source_type,
      disabled: readOnly,
      ...
      value: current.header.lieu_source_id,
      disabled: readOnly,
      onChange: e => patchHeader('lieu_source_id', e.target.value),
      style: {
        ...fStyle,
        flex: 1,
        fontSize: 11
      }
    }, (current.header.lieu_source_type === 'magasin' ? magasins : stations).map(l => /*#__PURE__*/React.createElement("option", {
      key: l,
      value: l
    }, l)))))), current.header.remarques ? /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11,
        color: '#888',
        marginBottom: 8
    ...
    }), current.header.remarques) : null, current.status === 'processing' && /*#__PURE__*/React.createElement("div", {
      style: {
        padding: 24,
        textAlign: 'center',
        color: BCSCAN_ORANGE,
    ...
    }), "Lecture du bon par l'IA\u2026"), current.status === 'pending' && /*#__PURE__*/React.createElement("div", {
      style: {
        padding: 24,
        textAlign: 'center',
        color: '#888',
    ...
    }, "En attente d'analyse\u2026"), current.status === 'error' && /*#__PURE__*/React.createElement("div", {
      style: {
        padding: 16,
        textAlign: 'center'
      }
    ...
    }, current.error || 'Analyse impossible'), /*#__PURE__*/React.createElement("button", {
      ...
      style: {
        padding: '6px 14px',
        borderRadius: 8,
        border: '1px solid ' + BCSCAN_ORANGE,
        background: '#fff',
        color: BCSCAN_ORANGE,
        cursor: 'pointer',
        fontSize: 12,
    ...
    }), "R\xE9essayer")), (current.status === 'done' || current.status === 'saved') && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("table", {
      style: {
        width: '100%',
    ...
    }, "Libell\xE9 lu"), /*#__PURE__*/React.createElement("th", {
      style: {
        padding: '6px 6px',
        textAlign: 'left',
        minWidth: 150
    ...
    }))), /*#__PURE__*/React.createElement("tbody", null, current.items.map((it, idx) => {
      const dispo = stockOf(it.article);
      ...
      return /*#__PURE__*/React.createElement("tr", {
        ...
        style: {
          ...
          background: exclue ? '#fafafa' : undefined,
          opacity: exclue ? 0.6 : 1
        }
        ...
        style: {
          fontWeight: 600,
          textDecoration: exclue ? 'line-through' : 'none'
        }
      }, it.article_lu || '—'), it.barre ? /*#__PURE__*/React.createElement("label", {
        style: {
          display: 'flex',
          alignItems: 'center',
          gap: 4,
        ...
        type: "checkbox",
        checked: !!it.reintegre,
        disabled: readOnly,
        onChange: e => patchItem(idx, {
          reintegre: e.target.checked
        }),
        style: {
          margin: 0
        }
      }), it.reintegre ? 'barrée sur le papier — réintégrée' : 'barrée sur le papier — exclue') : null, it.pile ? /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 9,
          color: '#aaa'
        }
      }, "pile : ", it.pile) : null, it.parcelle_lue ? /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 9,
          color: '#aaa'
        }
      }, "parc. lue : ", it.parcelle_lue) : null), /*#__PURE__*/React.createElement("td", {
        style: {
          padding: '4px 6px'
        }
      }, /*#__PURE__*/React.createElement("select", {
        value: it.article,
        disabled: readOnly,
        onChange: e => changeArticle(idx, e.target.value),
        style: {
          width: '100%',
          padding: '4px 6px',
          borderRadius: 6,
          border: artUnknown ? '2px solid #dc2626' : '1px solid #ddd',
          fontSize: 11
        }
      }, /*#__PURE__*/React.createElement("option", {
        value: ""
      }, "-- Article --"), articles.map(a => /*#__PURE__*/React.createElement("option", {
        key: a.id || a.nom,
        value: a.nom
      ...
      }, exclue ? null : statusDot(artUnknown ? 'unmatched' : it.article_status, it.article_alias_count))), /*#__PURE__*/React.createElement("td", {
        style: {
          padding: '4px 6px'
        }
      }, /*#__PURE__*/React.createElement("select", {
        ...
        disabled: readOnly,
        onChange: e => changeParcelle(idx, e.target.value),
        style: {
          width: '100%',
          padding: '4px 6px',
          borderRadius: 6,
          border: parcUnknown ? '2px solid #dc2626' : '1px solid #ddd',
          fontSize: 11
        }
      }, /*#__PURE__*/React.createElement("option", {
        value: ""
      }, "-- Parcelle --"), (() => {
        const cands = (it.parcelle_candidats || []).map(c => typeof c === 'string' ? c : c && (c.label || c.parcelle) || '').filter(l => l && knownParcelle(l));
        const uniq = [...new Set(cands)];
        if (!uniq.length) return null;
        return /*#__PURE__*/React.createElement("optgroup", {
          label: "Suggestions du scan"
        }, uniq.map(l => /*#__PURE__*/React.createElement("option", {
          key: 'cand-' + l,
          value: l
        }, nomOf(l))));
      })(), groupes.length > 0 && /*#__PURE__*/React.createElement("optgroup", {
        label: "Groupes"
      }, groupes.map(g => /*#__PURE__*/React.createElement("option", {
        key: 'grp-' + g.id,
        value: 'GRP::' + g.id
      }, g.label, " (", (g.membres || []).length, " parcelles)"))), useConsoSelector ? /*#__PURE__*/React.createElement("optgroup", {
        label: "Mes parcelles"
      }, refParcelles.map(p => /*#__PURE__*/React.createElement("option", {
        key: 'ref-' + p.label,
        value: p.label
      }, nomOf(p.label)))) : allParcelles.map(p => /*#__PURE__*/React.createElement("option", {
        key: p.Parcelle_Physique,
        value: p.Parcelle_Physique
      }, nomOf(p.Parcelle_Physique), " \u2014 ", cultureOf(p.Parcelle_Physique, p.Culture) || '?'))), it.ferme ? /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 9,
          color: '#888',
          marginTop: 2
        }
      }, it.ferme, " \u2014 ", it.culture) : null, /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 10
        }
      }, exclue ? null : statusDot(parcUnknown ? 'unmatched' : it.parcelle_status, it.parcelle_alias_count, 'la parcelle est la bonne'))), /*#__PURE__*/React.createElement("td", {
        style: {
          padding: '4px 6px'
        }
      }, /*#__PURE__*/React.createElement("input", {
      ...
      }, [...new Set(BCSCAN_UNITES.concat(it.unite ? [it.unite] : []))].map(u => /*#__PURE__*/React.createElement("option", {
        key: u,
        value: u
      ...
      }, it.article ? dispo : '—'), /*#__PURE__*/React.createElement("td", {
        style: {
          padding: '4px 2px'
        }
      ...
      }), nbExclues, " ligne", nbExclues > 1 ? 's' : '', " barr\xE9e", nbExclues > 1 ? 's' : '', " sur le papier, non enregistr\xE9e", nbExclues > 1 ? 's' : '', " \u2014 cochez la case pour la reprendre.");
    })(), !readOnly && blockers.length > 0 && /*#__PURE__*/React.createElement("div", {
      style: {
        marginTop: 10,
        padding: '8px 12px',
        background: '#fff5f5',
    ...
    }), "Impossible d'enregistrer : ", blockers.join(', '), "."), /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
    ...
    }), "Bon enregistr\xE9", current.numero ? ' — ' + current.numero : '') : /*#__PURE__*/React.createElement("button", {
      onClick: handleSave,
      disabled: !canSave,
      style: {
        padding: '8px 18px',
        borderRadius: 8,
        border: 'none',
        background: canSave ? 'var(--berry)' : '#bbb',
        color: '#fff',
      ...
      className: saving ? 'fa-solid fa-spinner fa-spin' : 'fa-solid fa-check',
      style: {
        marginRight: 6
      }
