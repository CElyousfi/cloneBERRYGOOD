    ...
    useEffect(() => {
      ...
      (async () => {
        for (;;) {
          ...
          try {
            const base64 = window.ImageDownscale
              ? await window.ImageDownscale.downscaleToDataUrl(next.file, { maxSide: 2000, quality: 0.8 })
            ...
            const resp = await fetch('/api/stock?action=scan-bc', {
              ...
              body: JSON.stringify({ scan_base64: base64, filename: next.file.name, type: type || 'engrais' }),
            });
            ...
            {queue.length > 0 && (
              <div>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 12, flexWrap: 'wrap' }}>
                  ...
                  {processing && <span style={{ fontSize: 12, color: BCSCAN_ORANGE }}><i className="fa-solid fa-spinner fa-spin" style={{ marginRight: 4 }}></i>Analyse en cours…</span>}
