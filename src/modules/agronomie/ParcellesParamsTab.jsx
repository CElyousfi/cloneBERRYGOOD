 ...
 * automatiquement par le wrapper window.fetch de bootstrap.jsx (/api/*).
 *
 * Props :
...
function pptFetchList(campagne) {
  return fetch('/api/pointage-rh?action=parcelles-params-list&campagne=' + encodeURIComponent(campagne))
...
function pptAssign(ref, campagneOrigine, campagneCible) {
  return fetch('/api/pointage-rh?action=assign-campagne-parcelle', {
    ...
    body: JSON.stringify({ ref: ref, campagne_origine: campagneOrigine, campagne: campagneCible })
  })
    .then(function (r) { return r.json(); })
    .then(function (json) {
  ...
  if (ha == null) {
    return React.createElement('span', {
      style: {
        ...
        background: '#FCEBEB', color: '#A32D2D', fontSize: 11, fontWeight: 600
      }
    }, '⚠️ manquante');
  }
  return React.createElement('span', {
    style: {
      ...
      padding: '2px 8px', borderRadius: 6, background: '#E1F5EE', color: '#0F6E56',
      fontSize: 11, fontWeight: 600
    }
  },
    React.createElement('span', null, Number(ha).toLocaleString('fr-FR') + ' Ha'),
    React.createElement('span', { style: { fontSize: 9, opacity: 0.7, fontWeight: 700 } }, 'BEE ONE')
  );
}
...
function ParcellesParamsTab(props) {
  ...
  function load(camp) {
    ...
    setErr(null);
    pptFetchList(camp)
      .then(function (json) { setRows(json.parcelles || []); })
      .catch(function (e) { setErr((e && e.message) || 'Erreur de chargement'); setRows([]); })
      .finally(function () { setLoading(false); });
  }

  useEffect(function () { load(campagne); }, [campagne]);

  function onAssign(row, cible) {
    if (cible === row.campagne_assignee) return;
    setSavingRef(row.ref);
    setErr(null);
    pptAssign(row.ref, campagne, cible)
      .then(function () { load(campagne); })
      .catch(function (e) { setErr((e && e.message) || "Échec de l'assignation"); })
      .finally(function () { setSavingRef(null); });
  }

  var thStyle = {
    textAlign: 'left', padding: '8px 10px', fontSize: 11, fontWeight: 700,
    color: PPT_C.textSecondary, textTransform: 'uppercase', letterSpacing: '0.03em',
    borderBottom: '1px solid ' + PPT_C.border, whiteSpace: 'nowrap'
  };
  var tdStyle = {
    padding: '8px 10px', fontSize: 13, color: PPT_C.textPrimary,
    borderBottom: '1px solid ' + PPT_C.surface3, verticalAlign: 'middle'
  };
  ...
  return React.createElement('div', { style: { padding: '4px 2px' } },
    ...
    React.createElement('div', {
      ...
      React.createElement('div', null,
        React.createElement('h2', {
          style: { margin: 0, fontSize: 18, fontWeight: 600, color: PPT_C.textPrimary }
        }, 'Paramètres Parcelles'),
        React.createElement('p', {
          style: { margin: '2px 0 0', fontSize: 12, color: PPT_C.textTertiary }
        }, 'Surface = BEE ONE (lecture seule). ' + (canAssign ? 'Assignation de campagne : DG/admin.' : 'Lecture seule.'))
      ),
      React.createElement('div', { style: { marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 } },
        React.createElement('label', { style: { fontSize: 12, fontWeight: 600, color: PPT_C.textSecondary } }, 'Campagne'),
        React.createElement('select', {
          value: campagne,
          onChange: function (e) { setCampagne(e.target.value); },
          style: {
            padding: '6px 12px', borderRadius: 8, border: '1px solid ' + PPT_C.border,
            fontSize: 13, fontWeight: 600, background: PPT_C.surface
          }
    ...
    err ? React.createElement('div', {
      style: {
        padding: '10px 14px', borderRadius: 8, background: '#FCEBEB', color: '#A32D2D',
        fontSize: 13, marginBottom: 12
      }
    }, err) : null,

    loading ? React.createElement('div', {
      style: { padding: 40, textAlign: 'center', color: PPT_C.textTertiary, fontSize: 14 }
    }, 'Chargement des parcelles…') :

    (rows.length === 0 ? React.createElement('div', {
      style: { padding: 40, textAlign: 'center', color: PPT_C.textTertiary, fontSize: 14 }
    }, 'Aucune parcelle avec du pointage pour cette campagne.') :

    React.createElement('div', {
      style: { overflowX: 'auto', border: '1px solid ' + PPT_C.border, borderRadius: 12, background: PPT_C.surface }
    },
      React.createElement('table', { style: { width: '100%', borderCollapse: 'collapse', minWidth: 820 } },
        React.createElement('thead', null,
          React.createElement('tr', { style: { background: PPT_C.surface2 } },
            React.createElement('th', { style: thStyle }, 'Ref'),
            React.createElement('th', { style: thStyle }, 'Label'),
            ...
            React.createElement('th', { style: thStyle }, 'Surface (Ha)'),
            React.createElement('th', { style: thStyle }, 'Campagne'),
            React.createElement('th', { style: thStyle }, 'Statut')
          )
        ),
        React.createElement('tbody', null,
          rows.map(function (row) {
            ...
            return React.createElement('tr', {
              key: row.ref,
              style: highlight ? { background: '#FFF8F0' } : null
            },
              React.createElement('td', { style: Object.assign({}, tdStyle, { fontWeight: 600, whiteSpace: 'nowrap' }) }, row.ref),
              React.createElement('td', { style: tdStyle }, row.label || '—'),
              React.createElement('td', {
                style: Object.assign({}, tdStyle, unknownFerme ? { color: PPT_C.red, fontWeight: 600 } : null)
              }, row.ferme || '—'),
              React.createElement('td', { style: tdStyle }, row.culture || '—'),
              React.createElement('td', { style: tdStyle }, row.variete || '—'),
              React.createElement('td', { style: tdStyle }, React.createElement(PptSurfaceBadge, { surface_ha: row.surface_ha })),
              React.createElement('td', { style: tdStyle },
                canAssign
                  ? React.createElement('select', {
                      value: row.campagne_assignee || row.campagne_derivee,
                      disabled: savingRef === row.ref,
                      onChange: function (e) { onAssign(row, e.target.value); },
                      style: {
                        padding: '4px 8px', borderRadius: 6, border: '1px solid ' + PPT_C.border,
                        fontSize: 12, fontWeight: 600, background: PPT_C.surface,
                        opacity: savingRef === row.ref ? 0.5 : 1
                      }
                    }, campagnes.map(function (c) {
                      return React.createElement('option', { key: c, value: c }, c);
                    }))
                  : React.createElement('span', { style: { fontSize: 12, fontWeight: 600 } }, row.campagne_assignee || row.campagne_derivee)
              ),
              React.createElement('td', { style: tdStyle },
