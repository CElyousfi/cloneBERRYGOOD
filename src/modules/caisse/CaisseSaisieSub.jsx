 ...
 *
 * Extrait du monolithe à comportement IDENTIQUE en mode création ; le mode
 * édition est l'ajout. Identifiants internes préfixés CSAI_.
 *
 * Props :
...
// Repli local (historique : le global pouvait ne pas être posé à temps).
...
function CSAI_types() {
  return TXN_TYPE_LABELS || CSAI_FALLBACK_TYPES;
}
...
function CSAI_campagneOf(dateStr) {
  if (CampagneUtils && CampagneUtils.campagneOf) {
    return CampagneUtils.campagneOf(dateStr) || '';
  }
...
function CaisseSaisieSub({ caisses, onDone, onCancel, defaultType, defaultCaisseId, editTx }) {
  ...
  const cultures = (CultureUtils && CultureUtils.CULTURES) || CSAI_CULTURES_FALLBACK;
