 ...
 *
 * Extrait de QuinzaineTab à comportement IDENTIQUE — règle de
 ...
 * finance/PivotAnalytiqueGrid.jsx, composant de présentation
 ...
 *   - PARCELLES_CULTURALES  (référentiel parcelles, agronomie)
 *   - sbParcelleHa()        (agronomie)
 *   - deriveSubFerme()      (shared)
 *   - PivotAnalytiqueGrid   (finance/PivotAnalytiqueGrid.jsx)
 * Déjà globales avant l'extraction : AnalytiqueUtils (shared/lib),
 * CampagneUtils, QuinzaineCampagneSelect, SB_PARCELLE_CAMPAGNE
 * (lue indirectement par sbParcelleHa), React (CDN).
 *
 * Props :
 *   analytiqueData  {Array}   Lignes analytiques de la quinzaine globale de
 *                             l'onglet (fetch + peuplement de
 *                             SB_PARCELLE_CAMPAGNE restent dans QuinzaineTab).
...
function AffectationAnalytiqueTable(props) {
          ...
          const _getAnalytiqueRowInfo = (r) => {
              ...
              if (ha === 0) {
                  ...
                  if (_fm) {
                      const _byFm = (PARCELLES_CULTURALES || [])
              ...
              if (ha === 0 && info.culture === 'Framboise') {
                  ...
                  if (_varFr && _fmFr) {
                      const _cand = (PARCELLES_CULTURALES || [])
                  ...
                  if (ha === 0 && _fmFr) {
                      ...
                      if (_secM) {
                          const _sec = _secM[1];
                          const _cand2 = (PARCELLES_CULTURALES || [])
          ...
          // Pivot délégué à la lib pure (shared/lib/analytiqueUtils.js) — regroupe les
          ...
          const _buildAnalytiquePivot = (rows) => {
              if (!AnalytiqueUtils) return { parcelles: [], groupedRows: [] };
              return AnalytiqueUtils.buildAnalytiquePivotByFamille
                  ? AnalytiqueUtils.buildAnalytiquePivotByFamille(rows, { detail: detailMode })
          ...
          const _analytiqueLocalCampagneCourante = (() => {
              ...
              // QuinzaineCampagneSelect (QCS_group : campagne DESC,
              ...
              // 2026-2027).
              const mostRecent = CampagneUtils && CampagneUtils.mostRecentCampagne
                  ? CampagneUtils.mostRecentCampagne(_analytiqueLocalPeriodeCampagne)
                  : '';
          ...
          const PivotGrid = PivotAnalytiqueGrid;
...
export { AffectationAnalytiqueTable };
