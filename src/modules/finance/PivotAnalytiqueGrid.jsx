 ...
 *
 * Extrait de finance/AffectationAnalytiqueTable.jsx (LOT 2a) à
...
var _pag_h = React.createElement;
...
function _pag_dash() {
  return _pag_h('span', { style: { fontSize: 10, color: 'var(--gray-400)' } }, '—');
}
...
function _pag_stack(metrics, valueOf, valueStyle, unitStyle) {
  ...
  metrics.forEach(function (m, i) {
    var v = valueOf(m, i);
    out.push(_pag_h('div', { key: 'v' + i, style: valueStyle }, v === null ? _pag_dash() : v));
    var unit = m.unit || '';
    out.push(_pag_h('div', { key: 'u' + i, style: unitStyle },
...
function PivotAnalytiqueGrid(props) {
  ...
  function totalCells(rendu, opts) {
    ...
    if (!multi) {
      return _pag_h('td', { style: opts.styleMono },
        _pag_stack(metrics, function (m) { return rendu(m); }, undefined, opts.unitStyle));
    }
    return metrics.map(function (m, i) {
      var st = { padding: opts.pad, textAlign: 'center',
      ...
      Object.keys(opts.styleMulti).forEach(function (k) { st[k] = opts.styleMulti[k]; });
      var v = rendu(m);
      return _pag_h('td', { key: '_total#' + i, style: st }, v === null ? _pag_dash() : v);
    });
  }
  ...
  function parcelleCell(row, pKey, ha, opts, colIndex) {
    ...
    if (!multi) {
      if (!cell) {
        return _pag_h('td', {
          key: pKey,
          style: { padding: opts.pad, textAlign: 'center', color: 'var(--gray-200)',
            borderRight: '1px solid #f5edf4', fontSize: opts.emptyFontSize },
        }, '—');
      }
      var style = { padding: opts.pad, textAlign: 'center', borderRight: '1px solid #f5edf4',
      ...
      if (onCellClick && _pag_cliquable(cell)) {
        ...
        attrs.onClick = function () {
          onCellClick({ parcelle: pKey, operationFamille: row.label, ha: ha, detailRows: cell.detailRows });
        };
        attrs.onMouseEnter = function (e) { e.currentTarget.style.background = '#fdf4f8'; };
        attrs.onMouseLeave = function (e) { e.currentTarget.style.background = ''; };
      }
      return _pag_h('td', attrs, _pag_stack(metrics, function (m, i) { return valeurs[i]; },
    ...
    return metrics.map(function (m, i) {
      ...
      if (actif) {
        ...
        a.onMouseEnter = function (e) { _pag_paint(e, debut, nbMetrics, '#fdf4f8'); };
        a.onMouseLeave = function (e) { _pag_paint(e, debut, nbMetrics, ''); };
      }
      var v = valeurs[i];
      return _pag_h('td', a, _pag_h('div', { style: opts.valueStyle },
        v === null ? _pag_dash() : v));
    });
  }
  ...
  var body = groupedRows.map(function (row) {
    ...
    if (row.type === 'operation') {
      return _pag_h('tr', { key: row.key, style: { background: '#fcfafc', borderBottom: '1px solid #f7f0f6' } },
        _pag_h('td', {
          style: { padding: padOperationLabel, fontSize: 11, fontWeight: 500,
            color: 'var(--gray-600)', position: 'sticky', left: 0, background: '#fcfafc',
            borderRight: '2px solid ' + color, zIndex: 1, borderLeft: '3px solid ' + color + '55' },
        },
          _pag_h('span', { style: { color: 'var(--gray-400)', marginRight: 6 } }, '↳'),
          row.label
        ),
        parcelles.map(function (p, i) {
          return parcelleCell(row, p[0], p[1], {
            pad: padOperation, fontSize: 11, emptyFontSize: 12,
            valueStyle: { fontWeight: 500, color: 'var(--gray-600)' },
            unitStyle: { fontSize: 9, color: 'var(--gray-400)' },
          }, i);
        }),
        totalCells(function (m) { return renderAgg(m, rowTotal(m, row), totalHa); }, {
          pad: padOperation,
          styleMono: { padding: padOperation, textAlign: 'center', fontWeight: 600,
            color: 'var(--gray-600)', background: '#fcfafc', position: 'sticky', right: 0,
            borderLeft: '1px solid #f0e6ef', fontSize: 11 },
          styleMulti: { fontWeight: 600, color: 'var(--gray-600)',
            background: '#fcfafc', fontSize: 11 },
          unitStyle: { fontSize: 9, color: 'var(--gray-400)', fontWeight: 400 },
        })
      );
    }

    // Ligne famille.
    return _pag_h('tr', { key: row.key, style: { background: '#fff', borderBottom: '1px solid #f0e6ef' } },
      _pag_h('td', {
        style: { padding: padFamilleLabel, fontWeight: 600, color: color, position: 'sticky',
          left: 0, background: '#fff', borderRight: '2px solid ' + color, zIndex: 1,
          borderLeft: '3px solid ' + color },
      },
        row.label,
        _pag_h('span', {
          style: { fontSize: 10, fontWeight: 400, color: 'var(--gray-400)', marginLeft: 6 },
        }, row.key)
      ),
      parcelles.map(function (p, i) {
        return parcelleCell(row, p[0], p[1], {
          pad: padFamille, emptyFontSize: 13,
          valueStyle: { fontWeight: 700, color: 'var(--gray-700)' },
          unitStyle: { fontSize: 10, color: 'var(--gray-400)' },
        }, i);
      }),
      totalCells(function (m) { return renderAgg(m, rowTotal(m, row), totalHa); }, {
        pad: padFamille,
        styleMono: { padding: padFamille, textAlign: 'center', fontWeight: 700, color: color,
          background: '#fdf4f8', position: 'sticky', right: 0, borderLeft: '1px solid #f0e6ef' },
        styleMulti: { fontWeight: 700, color: color, background: '#fdf4f8' },
        unitStyle: { fontSize: 10, color: 'var(--gray-400)', fontWeight: 400 },
      })
    );
  });
  ...
  return _pag_h('div', {
    ...
    _pag_h('div', {
      ...
      _pag_h('i', { className: 'fa-solid ' + (props.icon || ''), style: { color: color, fontSize: 14 } }),
      _pag_h('span', { style: { fontSize: 13, fontWeight: 700, color: color } }, props.title),
      _pag_h('span', { style: { fontSize: 11, color: 'var(--gray-500)', fontWeight: 400 } },
        parcelles.length,
        ' parcelle',
    ...
    _pag_h('div', {
      ...
      _pag_h('table', { style: tableStyle },
        ...
        _pag_h('thead', null,
          _pag_h('tr', { key: 'h1', style: { background: 'var(--gray-50)' } },
            _pag_h('th', {
            ...
            showTotal ? (multi ? _pag_h('th', {
              ...
              _pag_h('div', { style: { fontWeight: 700 } }, 'TOTAL'),
              _pag_h('div', { style: { fontSize: 10, color: 'var(--gray-400)', fontWeight: 400 } },
                totalHa > 0 ? (Math.round(totalHa * 100) / 100) + ' Ha' : 'Ha ?')
            ) : _pag_h('th', {
              title: note || undefined,
              style: { padding: '6px 10px', textAlign: 'center', fontWeight: 700,
                color: 'var(--gray-700)',
          ...
          multi ? _pag_h('tr', { key: 'h2', style: { background: 'var(--gray-50)' } },
            parcelles.map(function (p) {
              return metrics.map(function (m, i) {
                return _pag_h('th', {
                  key: p[0] + '#' + i,
                  ...
                  m.unit ? _pag_h('div', {
                    style: { fontSize: 9, color: 'var(--gray-400)', fontWeight: 400 },
                  }, m.unit) : null
                );
              });
            }),
        ...
        _pag_h('tbody', null, body),
        _pag_h('tfoot', null,
          _pag_h('tr', { style: { background: color + '18', fontWeight: 700 } },
            _pag_h('td', {
            ...
            parcelles.map(function (p) {
              ...
              if (!multi) {
                ...
                }, _pag_stack(metrics, function (m, i) { return totaux[i]; },
            ...
            totalCells(function (m) { return renderAgg(m, grandTotal(m), totalHa); }, {
              pad: padPied,
              styleMono: { padding: '8px 10px', textAlign: 'center', background: color + '28',
