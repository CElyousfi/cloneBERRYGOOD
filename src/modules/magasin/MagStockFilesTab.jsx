 ...
 *
 * Identifiants internes préfixés MSF_.
 *
...
var MSF_FARMS = [
  { key: 'berry_good', label: 'Berry Good' },
...
function msfTodayCasablanca() {
  try {
    ...
    var parts = new Intl.DateTimeFormat('en-US-u-ca-gregory-nu-latn', { timeZone: 'Africa/Casablanca', year: 'numeric', month: '2-digit', day: '2-digit' }).formatToParts(new Date());
    var f = {};
...
function msfNowHourCasablanca() {
  try {
    var fmt = new Intl.DateTimeFormat('en-GB', { timeZone: 'Africa/Casablanca', hour: '2-digit', hour12: false });
...
function msfFmtTime(ms) {
  if (!ms) return '';
  try {
    return new Date(ms).toLocaleString('fr-FR', { timeZone: 'Africa/Casablanca', hour: '2-digit', minute: '2-digit' });
  } catch (e) {
    return '';
  }
}
...
function msfApiGet(action, params) {
  return msfGetToken().then(function (token) {
    ...
    return fetch('/api/stock?action=' + encodeURIComponent(action) + (qs ? '&' + qs : ''), { headers: headers })
      .then(function (r) { return r.json().catch(function () { return { success: false, error: 'Réponse serveur invalide' }; }); });
  });
}
...
function msfUploadDirect(file, farm, dateStr) {
  ...
  return ref.put(file, { contentType: contentType }).then(function () {
    return { storage_path: path, filename: file.name || ('stock_file.' + ext) };
  });
}
...
function MSF_Dropzone(props) {
  ...
  var dragOver = dragState[0]; var setDragOver = dragState[1];
  ...
  return (
    <div style={{ background: MSF_C.surface, border: '1px solid ' + MSF_C.border, borderRadius: 12, padding: 16, flex: '1 1 260px', minWidth: 240 }}>
      <p style={{ fontSize: 15, fontWeight: 500, margin: '0 0 10px' }}>{farm.label}</p>

      {submitted ? (
        <div>
          <div style={{ background: '#E1F5EE', color: '#0F6E56', borderRadius: 8, padding: '10px 12px', fontSize: 13, marginBottom: 10 }}>
            <i className="fa-solid fa-circle-check" style={{ marginRight: 6 }}></i>
            Envoyé {status.submitted_at ? 'à ' + msfFmtTime(status.submitted_at) : ''}
            {status.submitted_by && status.submitted_by.name ? ' par ' + status.submitted_by.name : ''}
          </div>
          <label
            style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, color: MSF_C.textSecondary, cursor: busy ? 'wait' : 'pointer' }}
            title="Re-soumettre écrase le fichier du jour"
          >
            <i className={busy ? 'fa-solid fa-spinner fa-spin' : 'fa-solid fa-rotate'}></i>
            {busy ? 'Envoi…' : 'Re-soumettre'}
        ...
        <label
          ...
          style={{
            ...
            gap: 8, minHeight: 110, borderRadius: 10, cursor: busy ? 'wait' : 'pointer',
            border: '2px dashed ' + (dragOver ? MSF_C.green : MSF_C.borderStrong),
            background: dragOver ? '#E1F5EE' : MSF_C.surface2, color: MSF_C.textSecondary, textAlign: 'center', padding: 12,
          }}
        >
      ...
      {error ? <p style={{ color: MSF_C.red, fontSize: 12, margin: '8px 0 0' }}>{error}</p> : null}
    </div>
  );
}
...
function MagStockFilesTab() {
  ...
  var todayRow = useMemo(function () {
    return history.find(function (h) { return h.date === today; }) || null;
  }, [history, today]);

  function handleFile(farm, file) {
    setFieldErr(function (prev) { var next = Object.assign({}, prev); delete next[farm]; return next; });
    setBusy(function (prev) { return Object.assign({}, prev, { [farm]: true }); });

    msfUploadDirect(file, farm, today).then(function (up) {
      return msfApiPost('stock-file-submit', { farm: farm, storage_path: up.storage_path, filename: up.filename });
    }).then(function (res) {
      setBusy(function (prev) { return Object.assign({}, prev, { [farm]: false }); });
      if (res && res.success) {
        return loadHistory();
      }
      setFieldErr(function (prev) { return Object.assign({}, prev, { [farm]: (res && res.error) || 'Échec de la soumission — fichier non enregistré.' }); });
    }).catch(function (e) {
      setBusy(function (prev) { return Object.assign({}, prev, { [farm]: false }); });
      setFieldErr(function (prev) { return Object.assign({}, prev, { [farm]: (e && e.message) || 'Erreur réseau — fichier non enregistré.' }); });
    });
  }
  ...
  function cellContent(dateStr, farmStatus, isToday, farm) {
    ...
    return <span style={{ color: MSF_C.red }}><i className="fa-solid fa-circle-xmark"></i></span>;
  }

  return (
    <div style={{ fontFamily: 'system-ui, -apple-system, Segoe UI, Roboto, sans-serif', color: MSF_C.textPrimary, maxWidth: 920, margin: '0 auto' }}>
      <div style={{ marginBottom: 16 }}>
        <p style={{ fontSize: 19, fontWeight: 500, margin: 0 }}>Soumission Fichier Stock</p>
        <p style={{ fontSize: 13, color: MSF_C.textSecondary, margin: '2px 0 0' }}>Déposez le fichier stock du jour pour chaque ferme — Berry Good et Bahia. Un rappel WhatsApp est envoyé à 16h, 17h et 18h si un fichier manque.</p>
      </div>
        ...
        {MSF_FARMS.map(function (farm) {
          ...
          return (
            <MSF_Dropzone
              ...
              onFile={function (file) { handleFile(farm.key, file); }}
            />
          );
        })}
      </div>

      <p style={{ fontSize: 15, fontWeight: 500, margin: '0 0 10px' }}>Historique (30 derniers jours)</p>

      {err ? <div style={{ background: '#FCEBEB', color: '#A32D2D', borderRadius: 8, padding: '10px 14px', fontSize: 13, marginBottom: 16 }}>{err}</div> : null}

      {loading ? (
        <div style={{ background: MSF_C.surface2, borderRadius: 12, padding: '32px 24px', textAlign: 'center', color: MSF_C.textSecondary, fontSize: 14 }}>Chargement…</div>
      ) : (
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 420 }}>
            <thead>
              <tr style={{ textAlign: 'left', color: MSF_C.textSecondary, borderBottom: '1px solid ' + MSF_C.borderStrong }}>
                <th style={{ padding: '8px 6px', fontWeight: 500 }}>Date</th>
                <th style={{ padding: '8px 6px', fontWeight: 500 }}>Berry Good</th>
                <th style={{ padding: '8px 6px', fontWeight: 500 }}>Bahia</th>
              </tr>
            </thead>
            <tbody>
