 ...
 *
 * Dépendances :
 *   - ImageDownscale (shared/lib/imageDownscale.js) — compression avant
...
import * as BcScanMatch from '../shared/lib/bcScanMatch.js';
import * as ImageDownscale from '../shared/lib/imageDownscale.js';
...
function MagBCScanModal({
  ...
  const toBase64 = (file) => (ImageDownscale
    ? ImageDownscale.downscaleToDataUrl(file, { maxSide: 2000, quality: 0.8 })
  ...
  const saveEntry = async (entry, validItems, force) => {
    ...
    try {
      ...
      if (json && !json.success && json.doublon) {
        ...
        setDoublonScan({ ...json.doublon, message: json.error || '', entry, items: validItems });
        setSaving(false);
        return;
      }
  ...
  return (
    <div className="modal-overlay" style={{ zIndex: 9999 }}>
      ...
      {doublonScan && BCDoublonDialog && (
        <BCDoublonDialog doublon={doublonScan} saving={saving}
