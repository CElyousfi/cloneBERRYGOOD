...
function IMP_phraseSoldeSansMouvement(soldeAttendu, unite) {
  ...
  return IMP_fmtNum(s, 2) + (unite ? ' ' + unite : '')
    + ' en stock, aucun mouvement enregistré — ce solde provient de l’inventaire'
    + ' d’ouverture, non détaillé dans le grand livre.';
}
...
function IMP_Td(props) {
  return React.createElement('td', {
    style: { textAlign: props.align || 'left', fontSize: 12, color: props.color || IMP_C.textPrimary, padding: '6px 8px', borderBottom: '1px solid rgba(0,0,0,0.05)', fontWeight: props.bold ? 700 : 400 },
  }, props.children);
}
...
function InventaireMouvementsPopup(props) {
  ...
  var bounded = dateInventaire
    ? rawEntries.filter(function (e) { return !e.date || e.date <= dateInventaire; })
    : rawEntries.slice();
  var running = 0;
  ...
  var ecart = (soldeAttendu != null && isFinite(parseFloat(soldeAttendu)))
    ? Math.round((soldeFinal - parseFloat(soldeAttendu)) * 100) / 100
    : null;
  ...
  var card = {
    background: IMP_C.surface, borderRadius: 14, width: '100%', maxWidth: 760,
  ...
  var body;
  if (state.loading) {
    body = React.createElement('div', { style: { padding: 40, textAlign: 'center', color: IMP_C.textSecondary } }, 'Chargement des mouvements…');
  } else if (state.error) {
    body = React.createElement('div', { style: { padding: 40, textAlign: 'center', color: IMP_C.red } }, 'Erreur : ' + state.error);
    ...
    body = React.createElement('div', { style: { padding: 40, textAlign: 'center', color: IMP_C.textSecondary } },
      React.createElement('div', null,
        'Aucun mouvement pour cet article' + (lieuId ? ' au lieu ' + lieuId : '') + (dateInventaire ? ' jusqu’au ' + IMP_fmtDateFr(dateInventaire) : '') + '.'),
      phrase
        ? React.createElement('div', { style: { marginTop: 10, fontSize: 12, color: IMP_C.amber, fontWeight: 600, lineHeight: 1.5 } }, phrase)
        : null);
  } else {
    body = React.createElement('div', { style: { overflow: 'auto', padding: '0 16px 8px' } },
      React.createElement('table', { style: { width: '100%', borderCollapse: 'collapse' } },
        React.createElement('thead', null, React.createElement('tr', null,
          React.createElement(IMP_Th, null, 'Date'),
          React.createElement(IMP_Th, null, 'Type'),
          React.createElement(IMP_Th, null, 'Lieu'),
          React.createElement(IMP_Th, { align: 'right' }, 'Quantité'),
          React.createElement(IMP_Th, { align: 'right' }, 'Solde courant'))),
        React.createElement('tbody', null, rows.map(function (r, i) {
          var positif = r.quantite >= 0;
          return React.createElement('tr', { key: i },
            React.createElement(IMP_Td, null, IMP_fmtDateFr(r.date)),
            React.createElement(IMP_Td, null, IMP_typeLabel(r.type) + (r.numero ? ' · ' + r.numero : '')),
            React.createElement(IMP_Td, null, r.lieu_id || '—'),
            React.createElement(IMP_Td, { align: 'right', bold: true, color: positif ? IMP_C.green : IMP_C.red },
              (positif ? '+' : '−') + IMP_fmtNum(Math.abs(r.quantite), 2)),
            React.createElement(IMP_Td, { align: 'right', bold: true }, IMP_fmtNum(r.solde_courant, 2)));
        }))
      )
    );
  }

  var headerSub = articleNom + (lieuId ? ' · ' + lieuId : '') + (resolvedUnite ? ' · ' + resolvedUnite : '')
    + (dateInventaire ? ' · solde au ' + IMP_fmtDateFr(dateInventaire) : '');

  var footer = (!state.loading && !state.error && rows.length > 0)
    ? React.createElement('div', { style: { padding: '10px 18px', borderTop: '1px solid ' + IMP_C.border, background: IMP_C.surface2, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 } },
        React.createElement('span', { style: { fontSize: 12, color: IMP_C.textSecondary } }, rows.length + ' mouvement(s)'),
        React.createElement('span', { style: { fontSize: 13, fontWeight: 800, color: IMP_C.textPrimary } },
          'Solde final : ' + IMP_fmtNum(soldeFinal, 2) + (resolvedUnite ? ' ' + resolvedUnite : ''),
          (ecart != null && Math.abs(ecart) >= 0.01)
            ? React.createElement('span', { style: { marginLeft: 8, fontSize: 11, fontWeight: 600, color: IMP_C.amber } }, '(écart inventaire ' + IMP_fmtNum(ecart, 2) + ')')
            : null)
      )
    : null;
  ...
  return React.createElement('div', { style: overlay, onClick: onClose },
    React.createElement('div', { style: card, onClick: function (e) { e.stopPropagation(); } },
      React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 18px', borderBottom: '1px solid ' + IMP_C.border, background: IMP_C.surface } },
        React.createElement('div', null,
        ...
        React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 10 } },
          React.createElement('button', {
            onClick: function () {
              if (typeof XLSX === 'undefined' || rows.length === 0) return;
              IMP_exportExcel({
                rows: rows,
...
export { InventaireMouvementsPopup };
