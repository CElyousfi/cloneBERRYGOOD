 ...
 * ScanClientUpload.uploadDirectToPath (timeout dur 90 s, contentType
...
import * as ScanAttachmentUtils from '../shared/lib/scanAttachmentUtils.js';
...
function HsEmargementFooter(props) {
  ...
  var erreur = stateErr[0]; var setErreur = stateErr[1];
  ...
  function handleFile(ferme, ev) {
    ...
    if (!SCU || typeof SCU.uploadDirectToPath !== 'function') {
      setErreur({ ferme: ferme, msg: 'Module d\'upload indisponible — rechargez la page.' });
      return;
    }
    if (!SCU.isStorageAvailable()) {
      setErreur({ ferme: ferme, msg: 'SDK Storage non chargé — rechargez la page.' });
      return;
    }
    ...
    SCU.uploadDirectToPath(file, chemin).then(function (up) {
      return HSEF_token().then(function (tok) {
        ...
        return fetch('/api/primes?action=hs-emargement-submit', {
          ...
          body: JSON.stringify({
            periode: periode, ferme: ferme,
            storage_path: up.storage_path, filename: up.filename,
          }),
        }).then(function (r) {
      ...
      if (res && res.success) {
        ...
        setErreur({ ferme: ferme, msg: (res && res.error) || 'Échec inconnu — état NON enregistré.' });
      }
    }).catch(function (err) {
      setBusy('');
      setErreur({ ferme: ferme, msg: (err && err.message) || 'Erreur réseau — état NON enregistré.' });
    });
  }
  ...
  function handleView(ferme) {
    ...
    HSEF_token().then(function (tok) {
      var headers = {};
      if (tok) headers['Authorization'] = 'Bearer ' + tok;
      return fetch('/api/primes?action=hs-emargement-url&periode=' + encodeURIComponent(periode)
        + '&ferme=' + encodeURIComponent(ferme), { headers: headers })
        .then(function (r) { return r.json().catch(function () { return null; }); });
    }).then(function (d) {
      setBusy('');
      if (d && d.success && d.url) HSEF_openUrl(d.url);
      else setErreur({ ferme: ferme, msg: (d && d.error) || 'État introuvable.' });
    }).catch(function () {
      setBusy('');
      setErreur({ ferme: ferme, msg: 'Erreur réseau lors de l\'ouverture.' });
    });
  }
  ...
  var lignes = fermes.map(function (ferme) {
    ...
    return React.createElement('div', {
      ...
      React.createElement('span', { style: { fontSize: 12, fontWeight: 600, minWidth: 90 } }, ferme),
      React.createElement('span', {
        style: { fontSize: 11.5, color: depose ? '#0F6E56' : '#b26a00', fontWeight: 600 },
      },
        depose ? '✅ état signé déposé' : '⚠️ état signé manquant'
      ),
      depose ? React.createElement('button', {
        onClick: function (e) { e.stopPropagation(); if (!enCours) handleView(ferme); },
        disabled: enCours,
        style: {
          background: 'none', border: 'none', padding: 0, fontSize: 11.5,
          color: 'var(--blue, #1a73e8)', cursor: enCours ? 'wait' : 'pointer',
        },
      }, 'Voir') : null,
      ...
      React.createElement('label', {
        ...
        style: {
          ...
          padding: '3px 10px', borderRadius: 6, border: '1px solid #e67e22',
          color: '#e67e22',
        ...
        React.createElement('span', null, enCours ? 'Envoi…' : (depose ? 'Remplacer' : 'Déposer')),
        React.createElement('input', {
          key: 'file', type: 'file', accept: HSEF_ACCEPT, disabled: enCours || disabled,
          'data-no-fullscreen': '',
          onChange: function (e) { handleFile(ferme, e); },
          style: {
      ...
      entry && entry.filename ? React.createElement('span', {
        style: { fontSize: 10.5, color: 'var(--gray-400)' },
      }, entry.filename) : null,
      msg ? React.createElement('span', {
        style: { fontSize: 11, color: 'var(--red, #c0392b)', flexBasis: '100%' },
      }, msg) : null
    );
  });

  return React.createElement('div', {
    style: { marginTop: 16, paddingTop: 12, borderTop: '1px solid var(--gray-200)' },
  },
    React.createElement('div', {
      style: { fontSize: 12, fontWeight: 700, marginBottom: 6 },
    },
      React.createElement('i', { className: 'fa-solid fa-file-signature', style: { marginRight: 6, color: '#e67e22' } }),
      'États d\'émargement signés — ' + periode
    ),
    fermes.length === 0
      ? React.createElement('div', { style: { fontSize: 11.5, color: 'var(--gray-400)', fontStyle: 'italic' } },
        'Aucune ferme sur cette quinzaine.')
      : React.createElement('div', null, lignes),
    React.createElement('div', { style: { marginTop: 8, fontSize: 10.5, color: 'var(--gray-500)' } },
      React.createElement('i', { className: 'fa-solid fa-circle-info', style: { marginRight: 6 } }),
      'Une pièce par ferme : le scan de l\'état signé par le chef de ferme. PDF ou image, 25 Mo maximum.'
    )
  );
}

export { HsEmargementFooter };
