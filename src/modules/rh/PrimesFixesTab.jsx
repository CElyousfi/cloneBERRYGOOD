 ...
 *
 * Identifiants internes préfixés PFT_.
 *
 * Fonctionnalités :
...
import * as PrimesImportParse from '../shared/lib/primesImportParse.js';
...
// numKey : clé numérique canonique (idem QuinzaineTab) — ouvriers_registry est
...
// Recherche texte tolérante (matricule / nom). Réutilise une globale FilterUtils
// si présente, sinon helper inline équivalent (insensible à la casse, espaces).
function PFT_matchesQuery(text, query) {
  if (typeof FilterUtils !== 'undefined' && FilterUtils && typeof FilterUtils.matchesTextQuery === 'function') {
    return FilterUtils.matchesTextQuery(text, query);
  }
...
function PrimesFixesTab() {
    ...
    reader.onload = function (ev) {
      try {
        ...
        if (PrimesImportParse) {
          ...
          var res = PrimesImportParse.extractPrimesRows(aoa);
          if (res.error) {
  ...
  var addResults = useMemo(function () {
    if (!PrimesV2 || typeof PrimesV2.searchWorkers !== 'function') return [];
    return PrimesV2.searchWorkers(rows, addSearch, 50);
  }, [rows, addSearch]);
  ...
  var histView = useMemo(function () {
    if (!histRow) return [];
    if (!PrimesV2 || typeof PrimesV2.buildHistoryView !== 'function') return [];
    return PrimesV2.buildHistoryView(histRow.prime_history);
  }, [histRow]);
