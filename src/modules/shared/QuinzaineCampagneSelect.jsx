 ...
 *
 * Remplace les ~10 dropdowns de quinzaine dupliqués dans les écrans (Quinzaine, Coût
 ...
 * est ABSENT/vide, on dérive la campagne côté client via CampagneUtils
 ...
 *
 * Identifiants internes préfixés QCS_.
 *
 * Props :
...
function QCS_group(periodes, campMap) {
  ...
  return keys.map(function (c) {
    return {
      campagne: c,
      periodes: groups[c].slice().sort(function (a, b) { return QCS_num(b) - QCS_num(a); }),
    };
  });
}
...
function QuinzaineCampagneSelect(props) {
  ...
  if (props.includeEmpty) {
    children.push(React.createElement('option', { key: '__empty__', value: '' }, props.label || ''));
  }
  ...
  if (!hasAnyCampagne) {
    ...
    var flat = periodes.slice().sort(function (a, b) { return QCS_num(b) - QCS_num(a); });
    ...
    for (var g = 0; g < groups.length; g++) {
      ...
      var optChildren = grp.periodes.map(function (p) {
        return React.createElement('option', { key: (grp.campagne || '?') + '|' + p, value: p }, p);
      });
      children.push(
        React.createElement(
          'optgroup',
          { key: 'grp|' + (grp.campagne || '?'), label: grp.campagne || 'Autres' },
          optChildren
        )
      );
    }
  ...
  return React.createElement(
    'select',
    { value: value, onChange: handleChange, style: style, className: props.className },
    children
  );
}
...
export { QuinzaineCampagneSelect };
