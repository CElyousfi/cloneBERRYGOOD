 ...
 * automatiquement par le wrapper window.fetch (bootstrap.jsx) pour toute URL /api/.
 */
...
function BugReportButtonComponent(props) {
  ...
  function submit() {
    ...
    var body = {
      description: trimmed,
      screen: screenText,
      device: { userAgent: userAgent, viewport: viewport },
      profileId: profileId
    };
    ...
    window.fetch('/api/bug-reports?action=submit-bug', {
      ...
      .then(function (json) {
        if (json && json.success) {
          ...
          resetForm();
          setTimeout(function () { setOpen(false); }, 1200);
        } else {
  ...
  var floatBtn = React.createElement(
    'button',
    {
      type: 'button',
  ...
  var contextRows = [
    ...
    return React.createElement(
      ...
      React.createElement('span', { style: { color: '#666', minWidth: '92px', flexShrink: 0 } }, row[0]),
      React.createElement('span', { style: { color: '#222', wordBreak: 'break-word' } }, row[1])
    );
  });
  ...
  var modal = React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement('div', { style: { fontWeight: 700, fontSize: '16px' } }, '🐛 Signaler un bug'),
        React.createElement(
          'button',
          {
            type: 'button',
            onClick: closeModal,
            'aria-label': 'Fermer',
            style: { border: 'none', background: 'transparent', fontSize: '22px', cursor: 'pointer', color: '#888' }
          },
          '×'
        )
      ),
      ...
      React.createElement(
        ...
        'Description ',
        React.createElement('span', { style: { color: '#b3261e' } }, '*')
      ),
      ...
      photo && photo.dataUrl
        ? React.createElement(
            ...
            React.createElement(
              ...
              {
                type: 'button',
                onClick: function () { setPhoto(null); if (fileInputRef.current) fileInputRef.current.value = ''; },
                style: { display: 'block', marginTop: '6px', border: 'none', background: 'transparent', color: '#b3261e', fontSize: '12px', cursor: 'pointer', padding: 0 }
              },
              'Retirer la photo'
            )
          )
      ...
      React.createElement(
        'div',
        { style: { background: '#f6f6f6', borderRadius: '8px', padding: '10px', marginBottom: '14px', display: 'flex', flexDirection: 'column', gap: '4px' } },
        React.createElement('div', { style: { fontSize: '11px', fontWeight: 700, color: '#888', textTransform: 'uppercase', marginBottom: '2px' } }, 'Contexte (auto)'),
        contextRows
      ),
...
export { BugReportButtonComponent as BugReportButton };
