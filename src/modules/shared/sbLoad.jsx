/* Migré depuis public/app.jsx — extraction verbatim (non-régression).
   Module: shared | Déclaration(s): sbLoad */


// ===== RÉFÉRENTIEL PARCELLES SMART BERRY =====
        // sbParcelle.REF = { 'LABEL BEE ONE UPPERCASE' : { nom_sb, ha, ... } } — valeurs user-saved
        // sbParcelle.CAMPAGNE = { 'LABEL BEE ONE UPPERCASE' : sup (number) } — r.sup depuis campagne-list
        // Chargé une fois au démarrage. Mis à jour après chaque save. Voir aussi loadData() dans QuinzaineTab.
import { sbParcelle } from './sbParcelleState.js';

        function sbLoad() {
            fetch('/api/pointage-rh?action=sb-referentiel-list')
                .then(function(r) { return r.json(); })
                .then(function(d) {
                    if (!d.success) return;
                    sbParcelle.REF = {};
                    (d.parcelles || []).forEach(function(p) {
                        sbParcelle.REF[(p.label_bee_one || p.id || '').toUpperCase().trim()] = p;
                    });
                })
                .catch(function() {});
        }

export { sbLoad };
