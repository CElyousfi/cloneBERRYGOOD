 ...
 * tests : `partEcoulee` accepte un `CampagneUtils` implicite pour
...
import * as CampagneUtils from './campagneUtils.js';
 ...
 * campagne a UNE source de vérité (campagneUtils.js), la
 ...
 * dans ce fichier), avec un repli sur `CampagneUtils` pour l'appelant
...
function partEcoulee(args) {
  var a = args || {};
  // Injection d'abord (tests) ; sinon le module campagneUtils, présent dans le
  // navigateur. Hors navigateur sans injection : indéterminable → null.
  var utils = a.utils
    || (typeof window !== 'undefined' ? CampagneUtils : null);
...
export { CLASSES, JOURS_CAMPAGNE, QUINZAINES_PAR_MOIS, FENETRE_PAR_DEFAUT, MIN_QUINZAINES_CONSOMMEES, quinzaineNum, totalQuinzaines, quinzainesInfo, indexClasses, classeOf, classeFamille, resteBudgetCellule, moyenneMobile, resteRythmeCellule, decoreRestes, noteRestes, partEcoulee, joursEcoules };
