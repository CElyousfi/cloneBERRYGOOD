 ...
 *
 * ⚠️ CE FICHIER EXISTE EN DEUX EXEMPLAIRES :
 *      src/modules/shared/lib/coutMainOeuvre.js  (source de vérité, module ES)
 *      functions/lib/paie/coutMainOeuvre.js      (copie CommonJS)
 * Il n'a AUCUNE dépendance — `PaieUtils` est un argument — c'est ce qui rend la
 * copie possible, et donc vérifiable : même API et mêmes résultats sur un jeu
 * de cas (`functions/lib/paie/__tests__/coutMainOeuvre.parite.test.js`).
 * Modifier un seul des deux fait tomber la gate QA.
 *
...
export { CATEGORIES, coutFeries, nbJoursDistincts, masseSalarialeNette, netAPayer, categorieMO, primeFonctionADate, joursParOuvrier, paieOuvrier, netParCategorie, chargesSociales, coutEmployeur, totalQuinzaine };
