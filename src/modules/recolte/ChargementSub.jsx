...
function ChargementSub({ data, farmFilter, initialPeriode }) {
            ...
            const wList = (serverData?.chargementDetail || []).filter(w => w.periode === currentPeriode).map(w => ({ ...w, prime: w.jh * PRIME_CHARG })).sort((a, b) => b.jh - a.jh);
            const totalJH = wList.reduce((s, w) => s + w.jh, 0); const totalPrime = totalJH * PRIME_CHARG;
            const allDates = [...new Set(wList.flatMap(w => w.jours))].sort();
            const dailyData = allDates.map(d => { const ws = wList.filter(w => w.jours.includes(d)).length; return { date: d, workers: ws, cout: ws * PRIME_CHARG }; });
            return (<div className="fade-in">
                ...
                <div className="kpi-grid" style={{marginBottom:20}}>
                    <KPICard icon="fa-users" iconClass="berry" value={wList.length} label="Ouvriers Chargement" />
                    <KPICard icon="fa-calendar-check" iconClass="green" value={totalJH} label="Jours-Hommes" />
                    <KPICard icon="fa-coins" iconClass="orange" value={totalPrime.toLocaleString('fr-FR') + ' DH'} label="Total Prime Chargement" />
                ...
                <Panel title="Détail Prime Chargement par Ouvrier" icon="fa-list">
                    <table className="data-table" style={{fontSize:12}}>
                        ...
                        <tbody>
                            {wList.map((w, i) => (<tr key={w.matricule}><td style={{color:'var(--gray-400)'}}>{i+1}</td><td style={{fontFamily:'monospace',fontWeight:600}}>{w.matricule}</td><td><WorkerLink matricule={w.matricule} nom={w.nom} /></td><td style={{textAlign:'center',fontWeight:600}}>{w.jh}</td><td style={{textAlign:'right',fontWeight:700,color:'#8e44ad'}}>{w.prime.toLocaleString('fr-FR')} DH</td></tr>))}
                            <tr style={{background:'rgba(142,68,173,0.08)',fontWeight:700}}><td></td><td>TOTAL</td><td>{wList.length} ouvriers</td><td style={{textAlign:'center'}}>{totalJH}</td><td style={{textAlign:'right',color:'#8e44ad'}}>{totalPrime.toLocaleString('fr-FR')} DH</td></tr>
                        </tbody>
                    </table>
                </Panel>
                <Panel title="Prime Chargement par Jour" icon="fa-calendar-day">
                    <table className="data-table" style={{fontSize:12}}>
                        <thead><tr><th>Date</th><th style={{textAlign:'center'}}>Nb Ouvriers</th><th style={{textAlign:'right'}}>Prime (DH)</th></tr></thead>
                        <tbody>
                            {dailyData.map(d => (<tr key={d.date}><td>{new Date(d.date+'T12:00:00').toLocaleDateString('fr-FR',{weekday:'short',day:'numeric',month:'short'})}</td><td style={{textAlign:'center',fontWeight:600}}>{d.workers}</td><td style={{textAlign:'right',fontWeight:600,color:'#8e44ad'}}>{d.cout.toLocaleString('fr-FR')} DH</td></tr>))}
                            <tr style={{background:'rgba(142,68,173,0.08)',fontWeight:700}}><td>TOTAL</td><td style={{textAlign:'center'}}>{totalJH}</td><td style={{textAlign:'right',color:'#8e44ad'}}>{totalPrime.toLocaleString('fr-FR')} DH</td></tr>
                        </tbody>
                    </table>
                </Panel>
            </div>);
