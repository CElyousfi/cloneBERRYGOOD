/* Migré depuis public/app.jsx — extraction verbatim (non-régression).
   Module: qualite | Déclaration(s): ChefValidationBonsTab */
import { PROFILES } from '../shared/PROFILES.jsx';
import { useEffect, useState } from '../shared/reactHooks.jsx';

// ===================== CHEF: VALIDATION BONS APPORT TAB =====================
        function ChefValidationBonsTab({ currentProfile, profileData }) {
            ...
            const loadBons = async () => {
                ...
                try {
                    ...
                    for (const src of ['manual_entry', 'scan_ocr']) {
                        let query = db.collection('pfq_interne').where('source', '==', src);
                        if (viewMode === 'pending') {
                            query = query.where('status', '==', 'valide_qualite');
                            if (chefFerme) query = query.where('blocFerme', '==', chefFerme);
                        }
                        const snap = await query.limit(500).get();
                        snap.forEach(doc => {
                            const d = { id: doc.id, ...doc.data() };
                            if (viewMode === 'history') {
                                if (d.status !== 'valide' && d.status !== 'rejete_chef') return;
                                if (chefFerme && (d.blocFerme || d.ferme) !== chefFerme) return;
                            }
                            list.push(d);
                        });
                    }
                    ...
                    setBonsList(list);
                } catch (err) { console.error('Load chef validation bons error:', err); }
                setLoading(false);
            };
            ...
            const handleValidate = async (id) => {
                try {
                    ...
                    await db.collection('pfq_interne').doc(id).update({
                        status: 'valide',
                        validatedByChef: { profileId: currentProfile, name: profileData?.name || profileObj?.name || currentProfile },
                        validatedAtChef: firebase.firestore.FieldValue.serverTimestamp(),
            ...
            return (
                <div className="fade-in">
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16,flexWrap:'wrap',gap:8}}>
                        <h3 style={{margin:0}}><i className="fa-solid fa-clipboard-check" style={{marginRight:8,color:'var(--berry)'}}></i>Validation Bons d'Apport{chefFerme ? ' — ' + chefFerme : ''}</h3>
