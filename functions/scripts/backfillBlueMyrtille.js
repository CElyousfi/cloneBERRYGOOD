...
const TARGET_LIQ_NUMBERS = ["APIV-000086703", "APIV-000086893", "APIV-000087077"];
...
async function main() {
  ...
  for (const liqNumber of TARGET_LIQ_NUMBERS) {
    ...
    const pdf = await fetchPdfTextViaEmailId(data.emailId);
    if (pdf.error) {
  ...
  const backupPath = path.join(backupDir, `BACKUP-liquidations-blue-${stamp}.json`);
  ...
  for (const p of plans) {
    const updatePayload = {
      ...
      _backfill: { source: "backfill_blue_myrtille", updated_at: new Date().toISOString() },
