/**
 * analytiqueUtils.js — Pure helpers du pivot « Affectation Analytique — par Ha ».
 *
 * BACKEND COPY (duplication assumée). Source de vérité fonctionnelle :
 *   public/lib/analytiqueUtils.js (UMD front + node:test repo-root).
 ...
 *
 * POURQUOI ICI : `buildParcelleBudgetIndex` (campagneExportUtils) joint les
 * budgets saisis aux lignes de l'export par le code GB (`resolveGbCode`) et le
 * libellé normalisé (`opKey`). Ce sont ses INJECTIONS obligatoires : sans elles,
 * l'index retombe en mode dégradé et les 4 colonnes de budget sortent vides sur
 * les lignes d'opération. Le serveur doit donc disposer de la MÊME résolution
 * que le navigateur, au caractère près.
 *
 * Loaded as a plain CommonJS module from the backend :
 *   - require('./analytiqueUtils') → module.exports
