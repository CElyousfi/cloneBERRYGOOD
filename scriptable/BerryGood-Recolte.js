...
// =====================================================
//  Berry Good Farms - Widget Recolte
//  Total Kg + Rendement moyen par Equipe
// =====================================================
...
async function fetchData() {
  try {
    ...
    const workers = (json.workers || [])
      .filter(w => !logOps.test(w.operation))
      .sort((a, b) => (b.quantite || 0) - (a.quantite || 0));

    const totalKg = json.totalKgCueillette || Math.round(workers.reduce((s, w) => s + (w.quantite || 0), 0));

    // Par equipe: rendement moyen
    const byEq = {};
    workers.forEach(w => {
...
function buildWidget(data) {
  ...
  addSymbol(header, 'leaf.fill', 16, C.green);
  header.addSpacer(6);
  const titleCol = header.addStack();
  titleCol.layoutVertically();
  addText(titleCol, 'Recolte du Jour', Font.boldSystemFont(16), C.white);
  ...
  addText(badge, 'kg total', Font.systemFont(8), new Color('#ffffff', 0.7), 'center');
  ...
  for (let i = 0; i < Math.min(data.equipes.length, 10); i++) {
    ...
    const avgColor = eq.avg >= 20 ? C.green : eq.avg >= 10 ? C.orange : C.berry;
