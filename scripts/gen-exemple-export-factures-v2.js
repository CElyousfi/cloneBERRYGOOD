...
/**
 * gen-exemple-export-factures-v2.js — Génère docs/EXEMPLE-Export-Factures-TIMAC-25-26-v2.xlsx
 ...
 *
 * Usage : node scripts/gen-exemple-export-factures-v2.js
 */
...
const OUT = path.join(ROOT, 'docs', 'EXEMPLE-Export-Factures-TIMAC-25-26-v2.xlsx');
...
(async () => {
  ...
  scoped.forEach((f) => {
    ...
    if (!r.reconciled) { nbAnomalies++; anomalies.push({ num: f.numero_facture || f.numero, ecart: r.ecartTva, tvaBase: r.tvaBase, tvaClassee: r.tvaClassee }); }
  ...
  // --- Onglet Détail ---
  const detailHeader = ['N° Facture', 'Fournisseur', 'Date', 'Désignation', 'Quantité', 'PU HT', 'Montant HT', 'Taux TVA', 'Montant TVA', 'Montant TTC', 'Réconciliation'];
  const detail = [detailHeader.map(txt)];
  scoped.forEach((f) => {
    const { lines } = FE.buildFactureLines(f);
    lines.forEach((ln) => {
      detail.push([
        ...
        num(ln.montant_tva),
        num(ln.montant_ttc),
        txt(ln.reconciled ? 'OK' : 'Anomalie TVA'),
      ]);
    });
  });
  const detailCols = [{ wch: 16 }, { wch: 24 }, { wch: 12 }, { wch: 30 }, { wch: 10 }, { wch: 12 }, { wch: 14 }, { wch: 10 }, { wch: 12 }, { wch: 14 }, { wch: 14 }];
  ...
  console.log(`Flaggées (anomalie TVA)   : ${nbAnomalies}`);
  anomalies.forEach((a) => console.log(`   - ${a.num}: TVA base=${r2(a.tvaBase)} classée=${r2(a.tvaClassee)} écart=${r2(a.ecart)}`));
