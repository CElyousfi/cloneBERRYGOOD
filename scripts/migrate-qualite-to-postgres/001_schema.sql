...
COMMENT ON TABLE qualite.ecarts IS 'Écarts qualité — migrated from Firestore';
...
COMMENT ON TABLE qualite.bons_apport IS 'Bons d''apport — migrated from Firestore';
...
COMMENT ON TABLE qualite.liquidations IS 'Liquidations qualité — migrated from Firestore';
...
CREATE INDEX IF NOT EXISTS idx_qualite_bons_apport_ferme   ON qualite.bons_apport(ferme);
CREATE INDEX IF NOT EXISTS idx_qualite_bons_apport_date    ON qualite.bons_apport(date_bon);
...
-- 5. updated_at triggers (DROP IF EXISTS first so re-runs are safe)
DO $$ BEGIN
  DROP TRIGGER IF EXISTS trg_qualite_inspections_updated_at   ON qualite.inspections;
  DROP TRIGGER IF EXISTS trg_qualite_expeditions_updated_at   ON qualite.expeditions;
  DROP TRIGGER IF EXISTS trg_qualite_ecarts_updated_at        ON qualite.ecarts;
  DROP TRIGGER IF EXISTS trg_qualite_brix_updated_at          ON qualite.brix_readings;
  DROP TRIGGER IF EXISTS trg_qualite_bons_apport_updated_at   ON qualite.bons_apport;
  DROP TRIGGER IF EXISTS trg_qualite_pfq_updated_at           ON qualite.pfq_records;
  DROP TRIGGER IF EXISTS trg_qualite_liquidations_updated_at  ON qualite.liquidations;
END $$;

CREATE TRIGGER trg_qualite_inspections_updated_at
  BEFORE UPDATE ON qualite.inspections
  FOR EACH ROW EXECUTE FUNCTION qualite.update_updated_at();

CREATE TRIGGER trg_qualite_expeditions_updated_at
  BEFORE UPDATE ON qualite.expeditions
  FOR EACH ROW EXECUTE FUNCTION qualite.update_updated_at();

CREATE TRIGGER trg_qualite_ecarts_updated_at
  BEFORE UPDATE ON qualite.ecarts
  FOR EACH ROW EXECUTE FUNCTION qualite.update_updated_at();

CREATE TRIGGER trg_qualite_brix_updated_at
  BEFORE UPDATE ON qualite.brix_readings
  FOR EACH ROW EXECUTE FUNCTION qualite.update_updated_at();

CREATE TRIGGER trg_qualite_bons_apport_updated_at
  BEFORE UPDATE ON qualite.bons_apport
  FOR EACH ROW EXECUTE FUNCTION qualite.update_updated_at();

CREATE TRIGGER trg_qualite_pfq_updated_at
  BEFORE UPDATE ON qualite.pfq_records
  FOR EACH ROW EXECUTE FUNCTION qualite.update_updated_at();

CREATE TRIGGER trg_qualite_liquidations_updated_at
...
-- 7. RLS Policies (DROP IF EXISTS first so re-runs are idempotent)
DO $$ BEGIN
  -- inspections
...
-- ============================================================
-- Done. Expected: 7 tables, 14 indexes, 7 triggers, RLS on all.
-- ============================================================
